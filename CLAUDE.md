...
| **4a** | **Lighting — level** | Every location has a rig and a measured exposure, median-matched to its reference | **STALE AS OF 4l — DO NOT QUOTE.** `export_scene.ROOM_EXPOSURE` holds ONE CONSTANT PER ARCHETYPE, and it was solved when every room on the station was a single bay. After tiling, lengths INSIDE one key vary **31.6x** (`commerce`: `ngrath` 14.0 m .. `downbelow` 442.0 m), 16.0x (`generic`), 13.3x (`transit`), 9.7x (`store`), 7.4x (`industrial`). One value cannot light a 14 m shop and 442 m of Downbelow. Measured: `docking_bays` at 140 m renders **median x0.30 of `dock.webp`** against a x1.40 target and **92.25% crushed against the show's own 38.82%** — while p5 (x0.89), p95 (x0.89) and p5/p95 (x0.99) all PASS, so the SHAPE is right and only the LEVEL is wrong. The fix is a length term or a per-place solve, not a nudge — **and it cannot be done with the two knobs that exist**, which is the 4l finding. Measured A/B on `docking_bays`: at `--fixture-energy 3.0` (default) the frame is x0.30 with **92.25% crushed** but p5 x0.89, p95 x0.89 and p5/p95 **x0.99** — the show's own contrast ratio to within 1%. At `--fixture-energy 12 --ambient 0.35` the level improves to x0.51 and the crush falls to 57.14% (inside the show's 0.22–63.92% range), but **p95 collapses to x0.11 and p5/p95 to x8.44** — the image goes flat. Ambient owns the floor and destroys the highlights; fixture energy is nearly inert (this file already records 0 → 2.0 moving p5 by x1.0000). **The pendant floodlights in that room are EMISSION, and `emission_energy` is a material property that neither `room_exposure` nor `--fixture-energy` touches** — which is why the lights blow to white while the room stays black. A 140 m hall cannot be lit by the levers that light a 10 m one. **`--tonemap-exposure` DOES reach emission** — `export_scene.py` says so at its own emission finding, and at 3.2 it takes the median x0.30 -> **x0.88** with p95 and p99 both passing. But it lifts the FLOOR too: crush falls to **2.50% where the show has 38.82%**, so p5 goes x1.32 and p5/p95 x4.00. **THE MIDDLE EXPERIMENT IS CONFOUNDED AND ITS CONCLUSION IS WITHDRAWN.** `--fixture-energy 12 --ambient 0.35` moved TWO variables at once — fixture x4 and, once the arithmetic is done, ambient x3.9, because this room's default ambient is **0.089** (`AMBIENT_CALIBRATED_ENERGY 1.30 x AMBIENT_BY_ARCHETYPE["store"] 0.076 / AMBIENT_CALIBRATED_RATIO 0.30 x room_exposure 0.27`) and not the 1.30 the constant's name suggests. So *"ambient owns the floor and destroys the highlights"* is **not supported by it** — the frame did go flat, and which knob did it is unknown. **Ambient has never been raised on its own and that is the obvious untried experiment.** What survives is the two SINGLE-VARIABLE runs: defaults give the right ratio and too dark; `--tonemap-exposure 3.2` gives level nearly right and no blacks left.** Together they say the frame cannot be simultaneously bright at the top and deep at the bottom while the show's is (p95 0.2695 WITH 38.82% crushed). That is not an exposure problem: it is the room's lighting DESIGN — too much uniform fill, not enough hard localised source — and it belongs in `docking_bay.py`/`materials.py`, not in a gain. Previously: **20 / 21** in window (3z). The one that cannot be put in window is `plant`, and the cause is **quantisation, not geometry**: 85% of its frame sits at sRGB byte 0–1, under the eight-bit floor |
