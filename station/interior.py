...
def cell_manifest(schema, profile):
    ...
    # THE DRUM IS NOT A RING DECK AND IT STILL HAS GRAVITY. The loop above
    # skips every ring whose `kind` is not `deck_stack`, which is correct for
    # geometry -- the habitat drum is an open barrel with no ring corridor and
    # `deck.py` refuses to build it as a deck. But `deck_table` is not a
    # geometry table: it is what the ENGINE reads to know a floor's radius and
    # its field, and the drum has both. `walk.gd::_derive_omega2` looks up
    # (sector, ring_index, deck_index) and `stream.gd::bake()` refuses outright
    # without a row:
    #
    #     ERROR: bake: no deck_table row for green ring_index=1 deck_index=0
    #
    # So the Garden, the townscape, the tram, the spokes and the endcaps --
    # twelve of the register's 129 places -- could not be cut into cells at
    # all, and `tools/reach_gate.py` reported exactly those twelve as
    # unreachable with one cause.
    #
    # THE ROW IS DERIVED, NOT DECLARED. `drum_ground.FLOOR_R` is the radius at
    # which the drum turns 1.000 g and is the surface a body actually stands
    # on; the axial extent is the sector's own, the same `ex` every ring row
    # above uses. `cells` is 0 because this table's cell plan is the ring
    # corridor's 20-degree grid and the drum is not cut that way -- its cells
    # come from `stream.gd::bake` over the built mesh. Zero also keeps the
    # manifest's own "cell count matches the per-deck plans" assertion true.
    #
    # `decks` DELIBERATELY STAYS THE RING-DECK COUNT, because the selftest
    # above checks it against the number of `deck_stack` rings and that check
    # is right: this row is a gravity row for an open volume, not a deck stack
    # somebody forgot.
    drum_rows = []
    try:
        ...
        for sector in schema["sectors"]["extents_m"]:
            ex = schema["sectors"]["extents_m"][sector]
            for ri, ring in enumerate(ring_radii(schema, profile, sector)):
                if ring["kind"] != "open":
                    continue
                drum_rows.append({
                    "id": f"{sector}.{ring['id']}.d0",
                    "label": f"{sector.title()} habitat drum",
                    "sector": sector, "ring": ring["id"], "ring_index": ri,
                    "deck_index": 0,
                    "floor_r_m": _dg.FLOOR_R,
                    "floor_g": 1.0,
                    "use": "habitat",
                    "z0": ex["z0"], "z1": ex["z1"],
                    "cells": 0, "cell_deg": 360.0,
                    "cell_length_m": 0.0, "sight_line_m": 0.0,
                    "cell_triangles": 0,
                })
