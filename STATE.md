...
**Last updated:** 2026-08-07 · **Session 4s — A CONTAINER THAT RECYCLES IS A CONTAINER THAT
DELETES YOUR AGENTS' WORK, AND IT COST A RUN BEFORE THE RULE WAS WRITTEN. READ §26.**
This container recycled **three times**, each time rolling the checkout back several hours and
deleting every `git worktree`. The main agent lost **nothing** across all three, because every
increment had been pushed within a minute of landing and `git fetch && git reset --hard
origin/<branch>` restores a whole session. An **agent lost its entire run** — the collision-shell
fix, brief written and gate landed — because its brief said *commit on the branch* and it had
committed inside a worktree that then ceased to exist. **A commit in a container that rolls back
is a commit that never happened; a build agent's brief must say PUSH.** `scratchpad/push_mirror.sh`
is the standing defence and never creates a commit. · **AND THE FIRST BLIND-CRAFT WORKFLOW DIED OF
A HARNESS FAULT WHOSE ERROR MESSAGE NAMED THE WRONG THING** — all five agents burned 310k tokens
returning nothing, reported as `StructuredOutput retry cap exceeded`, which points at the schema.
The schema was fine: a misconfigured permission layer was stripping the parameters off **every**
tool call — `Read`, `Bash` (including a bare `pwd`), `Glob`, `Grep` **and `StructuredOutput`** —
so the agents could not work *and could not report that they could not work*. One diagnosed it
correctly in plain text and refused to manufacture gate numbers to fill the object. Transient, and
a probe agent confirmed the harness healthy afterwards. **Read the journal before believing a
workflow's failure line.** ·

**Last updated:** 2026-08-06 · **Session 4r (later) — P2 HAS STARTED: THE GAME CAN BE SAVED, HUNGER IS STATE, AND A SLEEP RUNS THE WORLD IT SLEEPS THROUGH. READ §25.8–25.11.** Three new player-facing systems and one new whole-station gate. **`coldstart.py --g8`**: save, walk 12 m away, advance the clock 5 h, spend 137 CR, add a bag item, load — position back **0.000 m**, clock **0.00000 h**, purse **0.00 CR**, bag **+0**, and the `--no-restore` control fails on all four. **`condition.py`** (PLY-06) and **`compress.py`** (PLY-05) close two of the four items R7 found in the spec and in no phase. **`deck.py --shell-fit`** is the new whole-station gate and it is RED at **20 rooms with geometry outside their own collision shell** — `ambassadorial_suites` builds 100.81 m of mesh across the ring and hands collision a 10.49 m shell. **`tools/aaa_gate.py` now runs in CI for the first time** at 59 structural errors (was 91; 32 were schema faults, closed at the rule). **Nothing on the station is at craft 1 any more** — the last three are at 3. · **Earlier in 4r — THE WALL A PLAYER CAN TOUCH IS NOT THE WALL A PLAYER CAN SEE. THIRTY-FOUR PLACES ARE BUILT OUTSIDE THE PRESSURE HULL, AND NOW A GATE SAYS SO. READ §25.1–25.3.** `deck.py --shell-fit` is the new whole-station gate and it asks CONTAINMENT rather than width — `ambassadorial_suites` builds 100.81 m of mesh across the ring and hands collision a **10.49 m** shell (10.4%), while the axis reads 100.0% on all thirty-two, which is the diagnosis: session 4k migrated one axis of `min(room_extent_m, bay_span_m)` to `rooms.built_span_m` and left the other. `tools/aaa_gate.py` now runs in CI for the first time (`saaa_selftest`, `saaa_scorecard`) at **59 structural errors**, down from 91 — 32 were schema faults and are closed at the rule (`_`-prefixed documentation keys, `resolved` as a real severity, `what_is_good` in the schema); the other 59 are review debt and **not one was edited away**. · **Earlier in 4r — THIRTY-FOUR PLACES ARE BUILT OUTSIDE THE PRESSURE HULL, AND NOW A GATE SAYS SO. READ §25.1–25.3.** `python3 station/interior.py --hull-fit` (CI `shullfit`, 2.0 s) is the whole-station question no other gate here could ask — every gate measures a place against a standard of its own kind, and **a room ninety metres outside the ship passes all of them**. **34 of 129 outside** (22 at their centre, 13 at one end of their own footprint only), plus 15 naming a deck number their stack cannot index. `cnc` and both observation domes by **101.6 m**. The cause is `ring_cells` never having taken a `z_m` while its sibling `ring_radii` did — *a fix applied to an instance and not to the rule*, one layer down. The mechanism landed with **no content moved** (`z_m` defaults to None); the flip needs a 23-minute rebuild and is task #3. **`mainstage_node` is fixed**: it was the only place with no home at ANY radius at its own z (Yellow waists to 18.3 m there, zero deck stacks) and moved z 3000→3250 for **zero change in radius, arc or gravity**. **INSTANCE TEN CLOSED**: the shipped build mounts `vista.gd` — and `--vista-gate` prints the honest part, that the boot cluster is **838 m along the axis from the nearest window**, so it still cannot fire. **Three of my own claims died to measurement this session and each is recorded where it was made**: the `deck_gap` fifteen do NOT build narrow rooms (0 of 15 — the bay span clamps them; it is `economy.floor_m2` that pays, 15 of 15 at +5.7%); the opened core hull does NOT exceed the envelope (that was two indexing conventions compared pointwise, 95.6 m apart at a step); and my first basis check **could not have failed** because `cnc`'s basis is symmetric to 1.2e-16. **A gate that passed with a dead control** — the vista transpose control took a MIN across three places and one symmetric basis dragged it to 0.0 m — is fixed and now names which bases cannot discriminate. `bootstrap.py` said *streaming cells: present* on **1 deck of 251**. · **Session 4q — TWO RULES REACHED THE GAME, AND THE WHOLE CORRIDOR CROWD WAS DRAWN MIRRORED. READ §24.** `consequence.certain_check` (who may enter a place) and `incident.py`'s collapse stream (who falls down, when, where, named) both had NO RUNTIME CALLER and now do, each with an engine gate that WALKS A BODY rather than scanning source — G4 `CHECK gate=PASS crossed=6 readings=6 admit=5 refuse=1` with the Vorlon berth refusing a citizen, G5 `COLLAPSE gate=PASS fell=1` with David Allan's INC-SICK promoting the nearest human out of the crowd 7.4 m away. Eight controls between them, all firing. **G4 caught me reproducing the defect inside its own fix** — the check landed in `hud.gd`'s mesh-name place branch and the shipped build STREAMS, so it would have been unreachable in the only build a player launches. **AND `ragdoll.gd` REFUSING A det=-1 TRANSFORM FOUND A SIX-SESSION-OLD DEFECT NO RENDER HAD**: `npc.gd::_walker_xform` built `Basis(fwd.cross(up), up, fwd)`, determinant exactly −1, so **every walker in the corridor was their own reflection** — while the BAKED half of the same crowd used a plain right-handed yaw. No gate here asks a transform whether it is a rotation. `player.gd` and `dialogue.gd` use the identical idiom and are both CORRECT (they pass `-fwd`); all five sites were checked, only npc.gd's three needed it. Ragdolls: 14 of 15 species, 16 segments, mass from the mesh's own volume split by its own skin weights, settling in 2.0–3.6 s at the deck's own 7.454 m/s² along a radius. `body.py` 1436/1436, `ragdoll.py` 24/24, `animation.py` 685/685 after a genuine one-argument bug (`rig` bound a skeleton and a skin from two different people). The drum floor is dressed — 1,945 features in 12 kinds — and §24.4b reads that frame honestly: fixed at distance, still bare underfoot. · **Session 4p — THE DIORAMAS ARE PEOPLE, AND THE OWNER'S AUDIT IS §22.** Room occupants now live by the station clock (66 of 66 change state over a day, proven in the engine on the streamed path), they sleep, `read` is the first verb with a consequence, the civic calendar exists, the Starfury docks, and structure went 4.34x -> 3.00x. **AND INSTANCE TEN: the shipped build had been instancing ZERO corridor walkers while this file quoted 963 — `main.gd` never set `crowd_glbs`.** The root cause of all of it is one sentence and it is the owner's own 4d ruling: the simulation is Python, the game is GDScript, and the bridge is ONE-WAY. · **Session 4n/4o — P1 IS THREE FOR THREE, THE LIVED-IN HALF IS BUILT, AND BUILT-BUT-UNREACHABLE IS FINALLY A GATE — WHICH IMMEDIATELY CAUGHT ME COMMITTING IT AGAIN.** See §21. **P1-G1 the job loop, P1-G2 the consequence ladder (34/34) and P1-G3 the incident generator (24/24) all landed and were verified here rather than taken from their agents' reports** — the station now has a job that pays, a standing you can lose, and **1,321 incidents a station-day in 16 classes with not one authored rate**. G3's honest shape is the next content target: **only 12 of 128 probe volumes clear 2 incidents/station-hour** — the arrival half and Downbelow are alive, the commercial ring is not (the Zocalo probe is 0.68/h) and the residential rings are at zero. The station gained **faction friction** (a Narn and a Centauri *cannot pass* in a ring corridor: 1.80 m wanted against 1.64 m available, so escalation falls out of the geometry — 30,250 encounters/station-hour, 7,963 world deltas, 1,400.4 m of displacement against a frictionless twin; 80 assertions across three gates, 9 of 9 content assertions FAIL on the pre-4n state) and the **navigation graph reached the engine** (741 of 741 walkable cluster pairs routed at run time in GDScript, node for node against `route_walk.path_between`, **0 mismatches**; a named resident walks 641.6 m of 645.0 m on the floor, 0 frames off it, arriving 1.43 m from target). **`tools/wiring.py` is the structural fix for this project's most-repeated failure** — finished, tested machinery with no caller on the shipped path, which had happened five times. It asks the rule's question instead of patching a sixth instance: does every `station/generated/…` path an engine script reads exist and get rebuilt by CI, and does anything import each tested module. **On its first run it found the sixth**: `main.gd` read `transit/transit_manifest.json`, **a name appearing exactly once in the repository — on the line that read it**. Nothing wrote it; `--build` writes `transit/lift.json`. `--mode=transit` could never have worked and its error message named a command that would not have fixed it. Then the **seventh** (`Director.route_between` returned empty unless `nav` was set and nothing set it — the nav agent had `main.gd` on its do-not-touch list, stopped at the boundary and handed over the patch, which is the process working but is exactly how one through six were born) and the **eighth** (`station/occluders.py`, 630 lines built specifically to close `budget.py`'s four-session red, imported by nothing; and `godot/project.godot` has no `use_occlusion_culling`, without which Godot ignores occluders entirely). **THE TOOL ALSO CORRECTED ITS AUTHOR TWICE, WHICH IS THE POINT OF IT.** I had checked the Starfury data with `ls station/generated/starfury/`, got "No such file", and written up "the three files do not exist". **They exist**, at `station/generated/scene/starfury/` — `main.gd` joins them onto a base of `…/generated/scene`, which an `ls` of a guessed directory cannot see and a scan that follows the actual read can. The real Starfury defect is weaker and still true: no CI step rebuilds that data, so it survives only because somebody once ran the builder by hand. And `--callers` reported **10 orphans of which 8 were false**, from two rules I wrote myself: `bespoke.py` dispatches as `__import__("shuttle")`, **a module name inside a string, invisible to an import regex — the identical trap CLAUDE.md already records from 4f**, and "outside its own directory" measures nothing when ~60 modules share `station/`. 10 → 2. **`docs/PLAYTEST.md` had four stale rows and all four erred in the costly direction** — saying ABSENT about dialogue, the day index, the Starfury and the economy, i.e. steering a human away from the three most interesting things the build does. *A doc that says a thing is missing goes stale in the direction nobody re-checks; an absence quietly becomes an instruction not to look.* · **Session 4k — L3 IS GREEN, AND THE 5.59 m WAS NEVER THE LIFT.** `python3 station/agenda.py --commute` passes at x1/x10/x60 at **0.05 m from the post** (was 5.59 m, deterministically, for a session) with all three controls FIRING at 38-42 m short; exit 0. `MASTER-PLAN` P0a's "resolve 0.05 vs 5.59 m" resolves to **0.05 m**, and the 0.05 is the 50 mm a body is stood above the shell so its settle drop can be asserted -- the body is ON its post. THE CAUSE, measured rather than argued: `agenda.room_legs` built the last leg of a route as three points -- the door, half a metre inside it, and the register's centre -- and that last hop is a STRAIGHT LINE laid when a room was an empty box. On `business_center` it passes through desk tops at r=219.06 (0.72 m up) and a floor-to-head partition, with point-to-triangle clearance under the 0.35 m capsule for 4.5 of its 5.5 m. **The body had arrived; there was nowhere further to go.** The tell was in the phase table, not the verdict: `walk_b` covered 50.7 m of a 56.41 m segment, and the missing 5.71 m IS the room leg (it now covers 57.3 of 57.45). TWO READINGS THIS CORRECTS, BOTH MINE: the target was never wrong (`roomnav.clear_at` puts a full capsule of room at the register's centre), and the route and the manifest already named the same point to 0.000 m -- so neither the aim point nor an endpoint disagreement was it. It was the APPROACH. NEW: `station/roomnav.py` -- where in a room a body can stand and the way in from the door, derived from the collision mesh the body collides with AND NOTHING ELSE (no second list of where furniture was meant to go). Height above the deck is `floor_r - r`; a triangle is the deck, over your head, or an obstacle dilated by the capsule; a `doorpanel_*` group is NOT a wall because `life.gd::_open_doors` opens it. Grid over the room's floor unrolled, BFS from the doorway, string-pulled path. Selftest 10/10 with three controls that fire. `--station` asks all 128 places the same question (a fix applied to an instance and not to the rule is a fix that will be needed again). In CI as `sroomnav`. Also: `assert_route_endpoints` fails if any of the five `room_target` call sites drops its mesh; `shell_groups` is built once for the GLB writer and the room search; `CAPSULE_R_M`/`CAPSULE_H_M` had three owners and now have one. KNOWN AND NOT FIXED: `route_walk.py --selftest` is 11/12 on a PRE-EXISTING hole in the circulation graph (`path_between` finds no route obs_rotundas -> earthforce_office, same sector, different decks); and `rooms.spawn_m` -- where the per-room walk test puts a body -- is derived from the room's declared `lateral_stack`/`fixtures_for` PLAN rather than from its emitted mesh, which is a second description of where the furniture is and the same class of defect one gate over. ALSO 4k, AND THE SAME DEFECT TWICE IN ONE SESSION: **P0.5 -- THE SHIPPED SCENE NOW STREAMS.** `godot --headless --path godot` with no arguments prints `main: STREAMED -- 18 cells, starting in cell 13`, three cells resident (154,454 tri against a 180,000 budget) with neighbours arriving at **+17.8 m and +56.1 m of lead**, `drop_m=0.053`, boot **8.9 s -> 3.7 s**. VERIFIED IN THE ENGINE BY THE MAIN AGENT, not taken from a report; `coldstart.py` G3 PASS, G1 PASS, controls pass. Gate `python3 station/boot.py --gate` is hermetic (fixture deck in a temp dir, no engine, ~40 ms, because `station/generated/scene/` is gitignored and a gate needing a built artefact could never run in CI), 10/10 with three controls; CI step `sthe_shipped_scene_streams`. **The plan knew only half of it:** `main.gd` never setting `cells_path` was one half, and the other was that **`walk.gd` called `_stream.update()` only inside `--stream-test`** -- so even with a path the shipped build primed one cell and never loaded a second, and walking to its edge ends the floor. `stream.gd` scored a green gate and moved nobody. FINISHED, TESTED MACHINERY WITH NO CALLER ON THE SHIPPED PATH -- the same shape as the L3 defect above, found twice in one session, and it is now this repository's most-repeated failure. The change also caught a regression IT CAUSED: `life.gd`'s Director binds people from the meshes in the tree AT THE MOMENT IT IS CALLED, which in a streamed build is the load screen -- `21 of 21 residents bound` became `1 of 21`, twenty people frozen at bake pose for ever. `main.gd::_rebind_on_stream` rebinds on `loads + frees` changing; live it goes 1 -> 6 as cells arrive. THREE RECORD CORRECTIONS: the cell baker is GDScript (`stream.gd::bake`), not Python; CI's "Streaming cell manifest" step is about the sight-line table the baker READS, not the cell set; and cells exist for **70 decks / 955 cells / 1.7 GB**, not "1 z-cluster of ~96". `docs/MASTER-PLAN.md` P0.5 is rewritten to the measured state with SIX items still open, each with its reason -- cluster-to-cluster hand-off untested, no RSS ceiling stated, the 1.7 GB payload not yet a budget with a consequence, the shipped cell set stale by 5,308 render triangles (collision matches exactly, so a collision-only freshness check would have called it fresh), `present_0300/1300` now measuring only the primed cell's cast (0 vs 1, was 4 vs 21), and `ambience.gd::bind` unaudited for the same bind-once defect. AND TWO MORE FINDINGS, BOTH FROM ASKING A QUESTION OF ALL 128 PLACES INSTEAD OF THE ONE THAT BROKE. **(a) THE CIRCULATION GRAPH WAS 71 DISCONNECTED PIECES IN EVERY CALLER BUT ITS OWN SELFTEST.** Over all 741 cluster pairs `path_between` routed **10** -- and those ten were exactly the same-deck pairs that need no lift; **0 of the 68 pairs `choose` can ever return**. A 100% failure was reporting as one broken pair. Cause: `RT.edges(nodes, schema)` left `profile=None`, `has_landing` handed None to `interior.ring_radii`, and a bare `except Exception` turned the TypeError into "this deck has no landing" -- **with a specific, plausible, entirely invented reason attached** (*"the column at z=6880 has no landing at deck 0: decks_in_ring returns a shorter stack there"*). It did not fail quietly, it manufactured evidence. `routes._selftest` was the only caller that both passed a profile and primed the module global, so it was green over a graph nobody else saw -- and it is the only one CI runs. **`agenda.py` was running on a five-piece station (66/70 lifts).** `routes.station()` resolves the pair once and caches it; the swallows are narrowed to (ValueError, KeyError, IndexError). Verified here: `routes.py --selftest` 5/5 -> **7/7** with a control that failed when written (0 lifts/71 components vs 70/1), `route_walk.py --selftest` 11/12 -> **12/12**, `--report` byte-identical. **Zero changes to route_walk.py** -- the evidence it is a rule fix. **(b) `roomnav.py --station`: 95 of 116 places a body can get into, 21 it cannot** (`docs/room-reach-4k.md`, exit 1). Its FIRST run passed 116 of 116 with fourteen wrong, because the criterion was "is anything standable reachable" and a 2x2 pocket beside a door is something -- **I shipped a gate that could not fail for the thing it is named after**, and had already pushed a change that made `room_target` return that pocket (a point OUTSIDE the room's wall) for those fourteen. Now room-relative, with two controls: a sealed partition reads POCKET and falls back to the register centre, THE SAME WALL with a doorway in it does not. The discriminator that stops it blaming the station for its own conservatism is the widest free run at four dilation radii -- same wall, same cluster, at capsule radius: `docking_bays` **0.80 m of clear doorway**, `vorlon_berth` **0.00 m** (and 0.00 at 0.20 m, so not a fat capsule). TWO SIGN BUGS, BOTH MINE, and the second still lives in code I did not fix: `agenda.room_legs` and `route_walk.legs_for` BOTH compute `place.z + room_interior_half_m` unconditionally, which is the FAR wall for any room on the other side of its corridor; nothing has counted how many. **AND THE OBVIOUS ENGINE A/B CANNOT SETTLE IT** -- `walk.gd`'s `--goto` steers STRAIGHT at a point, so driving it at `vorlon_berth` (40 deg round the ring) walked the body off a curved corridor: 1,661 m travelled, **1,084 of 1,800 frames in the air**. The valid experiment is to drive roomnav's own waypoints, which is what proved business_center at 0.05 m; wiring that into `walk_deck` upgrades the W-track deck gate from a straight-line steer into a real "can you get in" and is the obvious next increment. **AND THE STATION GOT BUILT: 926 m -> 8,014 m OF WALKABLE FURNISHED GEOMETRY (6.2% -> 53.9%).** `rooms.tiling` instances the representative bay along the footprint, which `bay_span_m`'s own docstring had promised and nothing did. **`docking_bays` 10.77 m -> 140.0 m**, render and collision agreeing, 70/70 floor probes. 19.6 M triangles for 8.7x the metres -- **2,450 per built metre, down from 6,354**. Gate `rooms.py --footprint` (CI `sfootprint`), shown FAILING on the old content (`--legacy`: 84 of 128 short) and asserting no two bays of a place hash identically. **FOUR PLACES THAT WERE GREY BAYS ARE PLACES**: `central_corridor` (Red's circulation spine, was a 6.93x6.00 m store room), `obs_rotundas`, `obs_dome_1/2` -- 4.28-6.16 objects/m2 against `zocalo` 2.48, all machinery gates PASS, 0 unbound materials. **THE FRAMES FOUND A DEFECT NO ASSERTION HAD**: the rotunda and both domes had NO WINDOWS -- INV-024's defect a third time, first caught by looking. Craft at HALF distance, honest: concourse 3, dome2 3, rotunda 2-3. **THE STATION IS STILL WALKABLE AFTER 8.7x**: `deck.py --sweep` 90/90 clusters, 128/128 locations, 0 floor holes, and the rooms went from 1,065 people to **2,805**. Ring-deck collision 58,660 -> 59,332 (+1.1%) because `collision.room_shell` tessellates around the arc only, never along z. **AND THE ROOM-REACH GATE WAS THE DEFECT, NOT THE STATION -- 115 of 116.** The count went 116 (criterion too weak, 14 wrong) -> 95 -> 99 (engine overruled my threshold on `mooring_clamps`) -> 101 (tiling) -> **115**, and the last step was one function: `Grid.snap` searched OMNIDIRECTIONALLY, so just inside a doorway it found the void inside a hollow wall or the vestibule beyond and called the room sealed. The tell was in the first run and I read it as geometry -- every failure at exactly `z_half - 0.1` from centre, 6.90/7.0 to 74.90/75.0, which is **the topmost cell of a 0.2 m grid: an arithmetic identity of my own gate**. No constant fixed it (0.5 m works for `business_center` and pockets `thieves_guild`; 2.5 m the reverse), which is what proved the MODEL wrong rather than the number. `Grid.step_in` walks inward from the door. **114 of 116 rooms need more than one waypoint to reach their own middle** -- the straight-line room leg was wrong almost everywhere and only failed visibly where a gate happened to walk. THREE HYPOTHESES KILLED, all mine, all blaming the station: missing apertures (engine walked 684 m into one), the vestibule (4 of 19), `built_span_m` overshooting the mesh (100.00 vs 100.36 -- matches). ONE REAL CANDIDATE LEFT: `vorlon_berth`, whose corridor doorway measures **0.00 m free at capsule radius where `docking_bays` in the same wall has 0.80 m** -- check `deck.build_collision`'s door cutting. OPEN: 794 m of vestibule (232 m `shuttle_car`, five on blue/0/0 because `docking_bays` pushes the corridor out) -- fix is growing rooms toward their door via a `deck.room_centre_z_m`, NOT attempted because the station is green and that is a six-file refactor with a 20-minute verify cycle; `--footprint` is a **23-minute** CI gate; 6 places overrun their triangle allowance (printed every run); the domes' windows render BLACK because `--shot interior` has no exterior environment; `bespoke.py` 206/207. **THEN THE BESPOKE HALF LANDED TOO: 11,248 m BUILT, FROM 926 m THIS MORNING — 12x.** The 37 `bespoke.NEAR_END` places went **625 m -> 3,668 m of 3,922 declared (16% -> 93.5%)**: `plant_zone` 13.81 -> **442.00 m**, `qtr_civilian` 5.22 -> **120.00 m** (and its corridor, which was an empty gap, is now built). Triangles 19.6M -> 29.4M = **2,616 per built metre**. **AND 7 OF THE 37 WERE NEVER BESPOKE** — `rooms.tiling` asked `place['module'] in NEAR_END`, the MODULE question, while `bespoke.composable()` exists because a module can own places it refuses by name; six exterior blisters and the corridor kit were built as generic rooms, pinned to one bay, **and excused from the gate as 'not ours'**. 1,024 m was misclassification. Session 4h's defect a third time: **the module is not the place.** `bespoke.AXIAL` now declares grow/one per module WITH ITS REASON, printed beside the metres — 13 places are deliberately NOT tiled (*'thirteen copies of C&C is not a bigger C&C'*) and report their own measured shortfall; `zocalo` is floored because growing it is 7.4x the frame allowance and *'a budget rule that shrank the best interior in the project to make its own number go green would be the gate deleting content'*. **A DEFECT NOBODY WAS LOOKING FOR:** `deck.room_shell_for` sized COLLISION from the generic bay for every composed place — `council_chamber`'s mesh is 22.38 m and its shell was **15.00 m**, `obs_dome_1` 16.55 against 6.29. Render geometry outside its own collision. **THE HARD RULE HELD AT 12x**: `deck.py --sweep` 90 clusters, 0 fail, 128/128, **0 floor holes**, people in rooms **1,065 -> 3,194**, generic module-owned 12 -> 10. **AND THE BEST INTERIOR THIS PROJECT HAS**: `station/shuttle.py` builds PLC-113's core shuttle car from LOCATIONS.md:364 (authority 1, `Babylon_5_2-22_35a.jpg`) item by item — red-maroon upholstery on moulded grey bases, amber panels low in the plinths, grab poles, recessed seams, the window band at seated eye height — every proportion in INV-294 carrying its pixel reading. **Craft 4 at half distance (my score, one ABOVE the builder's own 3)** — the first interior 4 outside the corridor kit. Its lighting was SOLVED against the reference rather than defaulted: `measure_frame --against` gave x2.70 of the reference median with p5 FAIL at defaults, x1.35 and **all seven distribution statistics PASS** at `--ambient 0.04 --fixture-energy 0.8`. What I caught that its solve did not: **15.85% of that frame is crushed**, the worst today, because the solve targeted the MEDIAN. Also built: `core_shuttle` as ONE of its 13 stations, per PLC-102's own ruling that the running tube is envelope and not rooms. **AND A REGRESSION I SHIPPED AND REVERTED**: deriving `test_materials_layer3.BESPOKE_ALL` from `bespoke.BESPOKE_GEOMETRY` took it 16 -> 12 modules and STILL missed the new three, silently dropping four others — there are THREE tables holding different sets and the new modules have **no module key anywhere**, being dispatched by place. No module-keyed derivation can find them; the gate must become place-keyed like the assembler already is (`docs/room-reach-4k.md` §12). **AND THEN THREE THINGS LANDED AT ONCE (4m).** **(1) SOMEBODY TALKS BACK** — the owner's 4d ruling, unmet since: **157/157 exchanges now offer a player line, 69 distinct player utterances, 120/157 press-yields**, verified in the shipped scene (`dialogue: 21 people can speak, of 21 in the cast`), three negative controls firing. Nothing in a line is written: *'17 minutes a circuit, 138 on duty'* is `security.beat('blue')` over 1,329 m and `security.on_duty(h)`; at 13:00 the same man says **174** and **59 pairs**. **AND `dialogue.gd` ALREADY EXISTED — 913 lines** — but `_wire_dialogue` is called from `_load_level()` eight lines ABOVE `_spawn_player()`, and its guard is `if _player == null: return`. **The node had never been built, on any path, ever.** Fourth instance this session of finished machinery with no caller on the shipped path. **(2) THE DOCKING BAY IS LIT: 7/7 PASS**, median x0.30 -> x1.48, crushed 92.25% -> 9.86%, verified here at x1.36. The diagnosis was ARITHMETIC — the corridor anchor delivers mean irradiance 4.2641 and the bay delivered **0.0722**, i.e. its forty floods were 1.7% of the anchor. **AND `measure_frame` CENSORS AT FLOOR=0.010**, so the old row passed its level test on **7.7% of its own pixels** — read `measurable %` beside any statistic from that tool. Craft 2 (was 1); not a 3 because 39 floods at a 35 deg cone put every point inside ~10 of them (an even sheet, not pools) and `floodlight()` hangs lamps BELOW the girder soffit so the truss takes no light. `obs_rotundas` deliberately NOT tuned: `rotunda.webp` is PLC-063's INWARD-facing room and PLC-064 faces OUT; the fix works (p5/p95 x6.79 -> x3.31) and was REVERTED because it decides C-003, which is OPEN and BLOCKING. **(3) THE SOAK PASSED: NO LEAK.** 300,000 frames, 85 min, **20,999.93 m walked, offfloor 0/300000**, 250 RSS samples, **first decile 521 MB, last decile 521 MB, drift +0.0%** (peak 818 transient). Its FIRST run was invalid and the failure is the lesson: it sampled `/bin/sh` at 6 MB and reported '+0.0% drift, no leak' — `godot_pids()[0]` took the wrapper. Now picks by largest RSS with a 50 MB floor that says THIS PROVES NOTHING when empty. P0a now: L3, CI wiring, CPU frame time (5.48 ms, 3.0x headroom), playtest script, soak — all DONE; only `--life-test` untouched. Previous headline: **Session 4j — THE COMPLETION BIBLE. READ docs/THE-STATION.md FIRST.** The owner rejected gate-talk and demanded exact specifics of everything the finished station contains. Delivered: the content bible (THE-STATION.md + spec/PLACES.md 128 places with tiling targets totalling 49,265 bays + spec/PEOPLE.md 28 factions/50-name cast/12 roles + spec/SYSTEMS.md 14 systems/9 surface programs + PLY player chapter + civic calendar) — ~330 checkable items, each with an acceptance check naming content. Adversarial panel: 68/72 findings confirmed, judge's 21 edit groups applied by a single-writer agent. THE MACHINERY EXISTS AND RUNS: tools/spec_registry.py (parses annexes → spec/completion.yaml, REFUSES ambiguity — first run found 179 real errors), station/spec_check.py (GREEN only via implemented harness; tool-to-build ⇒ RED by definition), CI step sspec_gate. Judge's verdict: after the edits the bible is real; without the machinery running, §1 is prose. NEXT SESSION: finish parser↔doc format binding (SHC/SUR/INC/PLY ID formats), get spec_registry --check to 0 errors, emit completion.yaml, adoption stamp per §9 (sha256 digest set incl. the machinery), then P0a per MASTER-PLAN 4i-FINAL (verify L3 — reproduced deterministic 5.59 m, cause: V1 furniture blocks room_target's register-centre aim in business_center; fix = standpoint-aware targets). L3 evidence in scratchpad/l3_repro.log. Previous headline: **Session 4h/4i** — deck.py --degeneracy closed the identical-rooms defect (98.7% different now); the 4i-FINAL plan adopted; walkable web slice at a URL; the L3 dispute; the trailer.** · **Session 4g** — · **Session 4h** — **READ §20.05 FIRST: the identical-rooms defect is closed. `deck.py --degeneracy` asks identity rather than similarity, found 5 modules drawing one room for 14 places, and all six now pass. The two frames that were byte-identical are 98.7% different** · **§20: the owner reassessed and the plan turned over — LIFE FIRST (60/30/10), the shell stays 1:1. Ashir walked 887.9 m to work; the variety gate is red at 27 indistinguishable clusters over 82 of 128 places; 857 of 857 residents commute across decks** · **Session 4g** — **THE STATION IS ONE WALKABLE PIECE. READ §19 FIRST: components 96 → 1, the lift exists, the whole station is built, and four green numbers were lies** · **Sessions 4e–4f** — **READ §14 FIRST. The renderer was wrong all session; layer 7 exists; the Starfury flies; 34 of 41 CI gates had not run for thirty pushes** · **4d** — **READ §8 FIRST: the owner asked what actually works and the answer was mostly no. Direction changed.** Also: 357/357 interactables, and the 60-minute engine tax is over · **4c** — the station is INTERACTABLE, the port is on a wall, and the 24-minute suites were one bad cache key · **4b** — a police force, friction in metres, the plated shell, the fitting-reach fix

## Session 4q — THE RULES REACHED THE GAME, AND THE WHOLE CROWD WAS MIRRORED

### 24. READ THIS FIRST — two rules a player now meets, and a six-session defect found by a gate

Two pieces of finished Python that the game had no way to reach are now reachable, each with an
engine gate that walks a body rather than scanning source. Both were the same shape, which is
this project's signature defect and now stands at **twelve instances**.

**P-05, the card check.** `consequence.certain_check` has decided who may enter a place since
P1-G2 — the six-rung identicard ladder, the arrest chain, visa revocation — and had **no runtime
caller**. A player could walk into the command deck of a military station unchallenged.
`boot.py::_checks` bakes the *result* for 98 of 129 places (`place -> {need, name, why}`), so the
engine never holds a copy of the rule; `hud.gd` fires it on the place transition it already
computes. G4 in `coldstart.py`:

    CHECK gate=PASS crossed=6 checked=6 readings=6 admit=5 refuse=1 silent=0 wrong=0
          tier=4(citizen) table=98
    vorlon_berth  need=5(accredited) tier=4(citizen)  result=refuse
                  why=P-05: a lift lobby serving function diplomatic_privilege

Four controls, each moves the verdict: `--no-checks` → 0 readings, `--tier=0` → 6 refusals,
`--tier=5` → 6 admissions, `--no-hud` → no reader. The plate is proven by a **frame**
(`docs/engine-4q-check-refused.png`, Forward+ on Vulkan 1.4 — the renderer named its own mode),
not by a grep.

**AND THE GATE CAUGHT ME REPRODUCING THE DEFECT INSIDE THE FIX.** `hud.gd::_where` has two
place-resolution paths — the level's mesh names when the deck loads whole, the interact sidecar's
boxes when it **streams** — and the check landed in the first alone. The shipped build streams.
`--check-gate` failed with *"this build named no place boxes"*, which is the entire argument for
a gate that walks a body. `_where` now has one exit and `_boundary` one call site.

**The collapse chain.** `incident.py` has decided who collapses, where and at what hour since
P1-G3 — a **named resident** as subject — into a ledger nothing read; `ragdoll.gd` could drop a
16-segment body only when `--ragdoll-gate` asked. Two finished halves, no join.
`incident.RAGDOLL_OF` (4 of the 30 classes put a body on the deck) → `visible_bodies()` →
`boot.py::_collapses` (**45 baked for the boot deck: 24 INC-SICK, 21 INC-STRAY, 5 species**) →
`main.gd::_fire_collapses` → `npc.gd::promote_walker`. G5:

    COLLAPSE gate: standing in docking_bays, clock wound to 00.31 for
      David Allan (human, INC-SICK) at 00.32
    ragdoll: PROMOTED human (David Allan) -- 16 segments, 63.7 kg, g=7.45 m/s2
    collapse: INC-SICK at docking_bays 00.32 -- a human goes down
      (the station's David Allan, human, the nearest human, 7.4 m away)
    COLLAPSE gate=PASS fell=1 nobody=0 live_ragdolls=1

The body comes **out of the crowd** — hidden from every MultiMesh bucket, off the collision
layer, restored on demotion because INC-SICK's subject gets up — and is matched to the incident's
species where the crowd has one. The line never borrows the incident's name for a body that is
not theirs: a corridor walker is anonymous by construction.

### 24.1 THE WHOLE CORRIDOR CROWD WAS DRAWN MIRRORED, FOR SIX SESSIONS

`npc.gd::_walker_xform` built `Basis(fwd.cross(up), up, fwd)`. `Basis(x, y, z)` takes **columns**
and is right-handed only when x × y = z; with `right = fwd × up` that product is **minus** fwd, so
the determinant is exactly **−1**. Every walker in the corridor was their own reflection, in all
three places `npc.gd` builds one.

**Nothing caught it because no gate here asks a transform whether it is a rotation**, and a
roughly symmetric body reads the same either way at corridor distance. `ragdoll.gd::promote`
refuses a mirrored transform — a check written for a bug the *gate* had hit — and that is what
surfaced it, on the first real promotion, measured: `det=-1.0000` before, `det=1.000` after.

**The two halves of one crowd disagreed about which way round a person is.**
`populace._place_body` places the baked crowd with a plain yaw, which is always right-handed.

**`player.gd` and `dialogue.gd` use the identical idiom and are BOTH CORRECT** — they pass
`Basis(right, up, -fwd)`, and the two negations cancel to +1. All five sites were checked rather
than the one that bit; only `npc.gd`'s three needed the other sign. *A fix applied to an instance
and not to the rule is a fix that will be needed again* — so this one was applied by asking the
question of every site.

`docs/engine-4q-crowd-handedness.png` shows the crowd still renders after the flip. **It is not
evidence about the mirror** — the mirror is invisible at corridor distance, which is why it
lasted six sessions. The determinant is the evidence.

### 24.2 Two more defaults nobody chose, both mine

* `ragdoll.gd` defaulted `g` to **9.81** and `up` to **+Y**, and the only caller that set them was
  the gate they were written in. `npc.gd::promote_walker` — the path a player's session actually
  goes through — set neither, so a real collapse would have fallen at Earth gravity straight down
  on a station whose deck delivers **7.454 m/s² along a radius**. The derivation moved into the
  director (`omega2`, off `cell_manifest.json`'s deck table); `--derive-g` withholds both and the
  run is **byte-identical** to the one that states them. INV-451.
* The ragdoll gate laid each body on the **player's** vertical while gravity pulled along the
  **body's own** — 0.8° over a 3 m offset at r=211.5 m. INC-ACCIDENT came to rest **123–127 mm
  into the deck** against the 10 mm the solver allows, reproducibly over two byte-identical runs.
  With both taken from where the body is, all three rest at 10.3–11.9 mm. *A 0.8° disagreement
  between where a thing is placed and where it is pulled is not a rounding error.*

### 24.3 A genuine one-argument bug in `animation.rig`

It built the skeleton from `body.nominal(species)` and handed the skin to
`costume.dressed_mesh` **without `ind=`** — which resolves its own figure through
`body.individual(species, npc_id)`. With `npc_id == NOMINAL` those are two different people. On
the Vree they diverge outright (stature 1.4833/build 0.7831 against the nominal 1.5000/0.7200 → a
24-vertex finger bound to an 18-vertex one); on the other thirteen the part counts happen to match
and the binding was merely computed from the wrong person's ring radii, **silently**. This also
affected `animation.emit()`, the committed skeleton/binding/clip export. `animation.py` 685/685.

### 24.4 What landed, with its evidence

| | evidence |
|---|---|
| `npc/body.py` skinning | `skin_selftest` reproduces the built mesh from the skinned mesh **vertex for vertex (worst 0 µm)** with a rotate-by-one control that must fail. **1436/1436** |
| `npc/ragdoll.py` | `--gate` **24/24**. Masses from the mesh's own volume split by its own skin weights land on standard adult segment fractions with nothing fitted: trunk 53.5% (adult ≈50), head 7.8% (≈8.1), one arm 5.6% (≈5.0), one leg 13.8% (≈16.1) |
| `ragdoll.gd` in the engine | settle 2.02–3.55 s, peak 3.35–5.26 m/s against free-fall-from-1.7 m-at-0.76 g of 5.03, rest penetration 10.3–11.9 mm on Godot's own 10 mm allowance. Controls: `--zero-g` fell=0.00 m; `--no-joint-limits` knees −180..180°; `--no-ragdoll` promoted=0 refused=3 |
| species | **14 of 15.** The Vorlon is the `column` plan — 5 bones, no legs, no arms — and `PLAN_BONES` already refuses it a gait for the same reason (INV-447). The brief had this backwards: `encounter_suit` is the **Gaim**'s plan and is fully humanoid |
| drum dressing | `drum_dressing.py` **189/189**, **1,945 features in 12 kinds** (708 town blocks, 265 trees, 100 copses, 79 hedgerows, silos, sheds, jetties, reeds, spires, lamps, gantries), LOD-resolved: 92,848 tri at z=5400, 60,020 at z=6300 — inside the 183,880 of headroom judge-4e measured as unspent. End-to-end through `export_scene.drum_parts`, 53 s, Forward+ |
| council chamber | 42/42, and the frames are real: the radiating fan, the spoked medallion on deep blue (canon — `Babylon_5` council reference, INV-171), the mosaic floor, the perforated bench front. **The delegate chairs are the weak element** — flat black lattice, no thickness variation |
| docking bay | 36/36 |

### 24.4b THE DRUM FRAME, READ HONESTLY — fixed at distance, still bare underfoot

`docs/engine-4q-drum-dressed.png`, standing at r=262 z=4700 looking down the axis. Against
judge-4e's finding — *"two flat colour fields meeting along a straight-edged polygon boundary …
nothing standing anywhere on 4.5 million m²"* — the **far half is closed**: settlement blocks
climb both walls of the barrel, tree masses break the arable parcels, lamps line the roads, and
the parcel patchwork now reads as land rather than as two colours.

**What the frame still shows and no assertion says:**

* **The near field is empty.** The bottom two-thirds of the frame is bare green and tan with one
  tree in it. The scatter is dense enough to read at 500 m and not at 20 m, which is the opposite
  of where a player stands. The LOD ladder resolves *detail* by distance; it does not place more
  things near the eye, and nothing measures features per m² **at walking distance**.
* **The near tree is still a lollipop** — a dark blob on a stick, at the distance where a player
  would see bark. That is the owner's *"sad excuse for a tree"* from session 3r, unchanged at
  close range: `copse` and `tree` were built to read as masses from across the barrel.
* **The parcel boundary is still a hard straight edge** where green meets tan in the foreground.
  `hedgerow` draws parcel edges in three dimensions on the far side; this one has none.
...
### 25.8 THE WALL A PLAYER CAN TOUCH IS NOT THE WALL A PLAYER CAN SEE — 19 of 32 rooms

**`python3 station/deck.py --shell-fit`**, control `--shell-fit --legacy`. Found by the council
chamber's craft agent as a BLOCKING robustness finding on one room, measured by the main agent
across all of them, and it is thirty rooms rather than one.

`station/deck.py` sizes a room's collision shell with `min(rooms.room_extent_m,
rooms.bay_span_m) / 2` in **two** places — `room_shell_for`, which is the shell a body collides
with, and `room_half_w_m`, which `deck_plan`'s door-fit test and `navgraph_export` both use.
Session 4k fixed exactly this expression **for the axis** and left the width. Its own docstring
on `room_interior_half_m` says why the axis mattered — *"a shell sized on the old expression
would put an INVISIBLE wall 10.8 m into a 140 m room a player can see the whole length of"* —
and `rooms.built_span_m` exists because *"three places in `deck.py` each carried their own copy
of the old answer"*. **One of its two return values never reached either site.** A fix applied
to one axis and not to the rule.

| place | shell | its own mesh | | shell ceiling | mesh height |
|---|---|---|---|---|---|
| `ambassadorial_suites` | 10.49 m | **100.81 m** | **10.4%** | 3.40 | 3.04 |
...
**Two are wrong the other way and let a player walk OUT through a wall** — `obs_dome_2` 9.60
against 8.76 (109.5%) and `shuttle_car` 6.40 against 4.78 (133.8%). **And the axis reads 100.0%
on every single one of the thirty-two**, which is the whole diagnosis in one column: this is one
expression half-migrated, not thirty independent bugs. *Read the shape of a failing number
before reading its size.*

**THE GATE ASKS CONTAINMENT, NOT WIDTH, AND THAT IS NOT A REFINEMENT.** `bespoke.room_shell`
recentres a module's x **on the doorway** — its own comment says *"local x = 0 is not a centre,
it is a DOORWAY, and the bounding box only coincides with it when a module happens to be
symmetric. Two are not"* — while `collision.room_shell` takes one half-width symmetric about the
place's bearing. **A room can be given exactly the right width and still hang out one side.** So
the gate asks whether any vertex of the render mesh lies outside the arc the shell spans, which
is the thing a player actually walks into.

**AND THE GATE IMMEDIATELY PROVED ITS OWN DESIGN, which is what a real gate does.** Run over 129
places (7 min) it reports **20 rooms with geometry outside their own collision shell** — one
more than the width table finds, and the extra one is the argument: `kosh_quarters` is 10.22 m
of shell for a 10.66 m mesh, **95.9%, which passes any width test**, and its mesh runs
−2.25 .. +8.41 m, so **3.30 m of that room is outside a shell of very nearly the right size.**
The worst is `ambassadorial_suites`: shell ±5.25 m, mesh −92.28 .. +8.53 m — **87.04 m outside
on the left**.

**The other two checks pass today, and that is information rather than slack.** `SPAN` is 0/129
because `rooms.built_span_m` currently returns the same wrong number the two `deck.py` sites
compute — so the fix is to make *that one function* truthful and let `SPAN` fail until both
sites follow it. `FOOT` is 0/129 because no room's `built_span_m` exceeds its footprint. CI step
`sshellfit`.

**What makes widening safe is measured rather than hoped for.** `scratchpad/shellfit2.log`:
**31 of 32 composed meshes sit inside their own declared footprint** — `zocalo` builds 5.74° of
a 70° footprint, `plant_zone` 1.83° of 360°, `downbelow` 1.75° of 180°. `directory.collisions()`
already asserts footprints do not overlap, so **a shell that stays inside its footprint inherits
non-overlap by construction** and no pairwise shell test is needed. The gate asserts it.

**The one exception is `qtr_transient`**: 31.09° of mesh against a 26.00° footprint, 5.09°
outside its own declared arc. Checked against every place sharing its ring, deck and z band —
...
### 25.11 P2's OTHER TWO — hunger as state, and a sleep that runs the world

**PLY-06, `python3 station/condition.py --selftest` (16/16), CI `scondition`.** Ruled LIGHT by
the owner, and the row does something unusual: it enumerates the **complete** effect set and
forbids anything outside it without a SPEC-CHANGE. Two effects, total — fed/hungry moves the
dialogue band one step, rested/tired earns or forfeits a pay-stub bonus, and anything worse
produces *nothing*.

**The gate is a whole-state diff, which is the spec's own choice and a better one than I would
have picked.** A checklist can only fail for a MISSING effect; a diff fails for an **extra**
one, which is the failure mode a mild system actually has — it grows. The self-test plants an
undeclared `screen_blur` key in a copy of the control run and asserts the diff catches it;
without that, the harness would be a whole-state diff in name only. Human, two station-days:
**446 differences, falling in exactly {states, warmth_band, pay_bonus}**; first divergence at
t=8.25 h, pay bonus lost at t=16.75 h; **41 days** with no food or sleep is still exactly those
two effects at exactly those two values.

**Every window comes from `npc/schedule.py`, checked by DISCRIMINATION rather than by import** —
a two-meal species must get a longer interval than a three-meal one, which a constant in the
condition model cannot produce. A Vorlon has `meals: ()` (*"nothing has ever shown a Vorlon
eat"*) and is never rated hungry rather than rated hungry immediately. INV-660.

**And it has two real consumers, asserted end to end**, because a condition model nothing reads
is instance twelve: `dialogue.register(..., condition=+1)` moves a fed player's band `blunt` →
`plain`, and `economy.pay(..., rested=True)` turns a 50 CR shift into 52 with the 2 CR as a
**named `(rested bonus)` line on the stub** — a forfeited bonus a player cannot see is a rule
that is not in the game. 4%, INV-661, small on purpose: a bonus worth optimising around would
make sleep a resource, which the enumerated effect list exists to prevent.

**PLY-05, `python3 station/compress.py --selftest` (14/14), CI `scompress`.** *"Advance the
station clock at compressed rate **through the running simulation** — events still fire, stocks
still move, the world does not pause."*

**What shipped was a jump.** `interact.gd::_sleep` called `Clock.set_hour(wake)` once. Fifteen
honest lines against a system that already existed — and the seven hours between lying down and
waking **never happened**. Nothing fired in them, and nothing could wake you, because there was
no interval to be woken during.

It steps now, one station-hour at a time, in both places. Sleeping 22:00 → 05:15 in Downbelow:
**7 station-hours stepped, 90 events fired while asleep, and the sleeper was WOKEN at 05:00 by
an INC-SWEEP in the room they were camping in** — PLY-05's own CHECK case happening for real
rather than planted.

**MY FIRST NEGATIVE CONTROL WAS WRONG AND THE CORRECTION IS THE USEFUL PART.** It asserted a
single-step advance would fire FEWER events than eight hourly ones — *"a jump has no
interior"*. It fired **110 against 95**, because `incident.simulate` honours whatever window it
is handed. **Event count is not a proxy for "the world ran".** What compression buys is
**resolution**: a one-step advance checks for a waker once, at the end, so a 03:00 sweep during
a 22:00–05:15 sleep can only wake you at 05:15, which is not being woken. The control measures
decision points now — 1 against 7 — and the wrong version is kept in the file with its numbers.
INV-662, INV-663. The three interruption sources PLY-05 names and this station has no source for
(PA emergency, rent day, booked appointment) are printed on every run with the reason each is
absent, rather than silently unchecked and reported as a clean night.

**AND `station/npc/body.py` WAS A FOURTH COPY OF THE SCREEN MODEL — the only one that ASSERTS.**
It restated `FOV_DEG = 50.0` and checked `hull_lod.FOV_DEG == FOV_DEG` with exactly the right
reason (*"two chains with two budgets pop differently in one frame"*). When 4r derived the lens
off `player.gd`, the three copies that READ moved and this one did not, so it fired — correctly,
on the only file in the project that could have caught it. **The fix is not to edit the literal,
it is to stop having one.** It reads `lod.py` now, with a stated fallback.

That moved a real number: **`SUBPIXEL_FIGURE_M` 695 m → 423 m**, because a wider lens puts fewer
pixels on a metre and a 0.45 m shoulder goes sub-pixel nearer. The assertion guarding it read
`> 500 and < 900` — *a window remembered from the 50° lens, which is the same defect one level
up*. It checks the value against the screen model's own arithmetic at whatever lens is in force
now. body.py **1,435/1,436 → 1,436/1,436**.
...
nowhere in `.github/workflows/validate.yml`. `docs/aaa-scorecard.json` — the layer-8 authority —
was therefore unchecked: a round could cite a descriptor that does not exist, score a dimension
with no evidence, or lose its `findings` to a typo, and the dashboard would still print. It runs
now as `saaa_selftest` + `saaa_scorecard`, and the final aggregating step checks **all 75** step
ids with none unlisted.

**It is RED at 59, and that is the point.** 32 of the original 91 were the schema being wrong
rather than the reviews, and those are closed at the rule:

* **28** — `evidence` was keyed only by dimension, so a review had nowhere to record the things
  that back several dimensions at once (which renderer, which frames, the measured
  distribution). Those take a `_` documentation prefix now, at every level of the file. The
  allowance is deliberately narrow: `_x` is documentation **only when `x` is not itself a schema
  key**, because `_findings` beside a missing `findings` is a round whose findings have silently
  vanished and which then reads as clean. There is a self-test for exactly that.
* **4** — `resolved` is a real severity now. Three `exterior_approach` rounds had closures to
  record and no severity to record them under; filing a closure as a `note` makes it
  indistinguishable from an open preference. It is exempt from the score coupling and from
  nothing else — six self-tests, including that it cannot explain a below-bar score.
* `what_is_good` is in the schema rather than tolerated. A review that enumerates only defects
  gives a builder no way to know what it must not break while fixing them.

**The remaining 59 are review debt and not one was edited away**: 19 dimensions scored below the
bar with no `major` naming the descriptor that fails, 15 scores of 4+ with no evidence string, 8
findings whose descriptor is `-`, 8 citing a descriptor their own round's score contradicts, 5
severity/score mismatches, 4 at-or-above-bar scores carrying a major. Each belongs to the
reviewer who wrote that round. Rewriting their scores to go green is the convenient reading.
Recorded in the scorecard's own `notes` so the red is readable rather than ambient.

**Council chamber merged: craft 1 → 2 → 3**, and neither new round is clean — round 2 opens with
the BLOCKING shell finding above and round 3 carries it forward unfixed. Both are the build
agent's own, so per AAA-STANDARD neither advances the clean-round counter.
...
* A promoted body is **4 draw calls**, not 1, and the crowd already spends 31–33 of a 32 budget.
  Four concurrent ragdolls is 16 more. Reported by `concurrent_cap()` rather than assumed away.

## Session 4p — THE OWNER'S AUDIT, AND THE DIORAMAS ARE PEOPLE

### 22. READ THIS FIRST — the owner asked what was being glossed over, and the answer was four things

He asked, in order: does the plan cover transports and processing; how many NPCs are there and
what do they do; does everything have collisions; do they sit, sleep and move; can a person be
traced to a home and a job; how much of the ship is built; are the unique places hand-made; does
the Garden still look like cubes; is the station actually *functional* — *"can be messed up and
have real effects"*.

**What checked out**, measured rather than recalled: **55 transport arrivals and 1,734 souls a
day** through ten sourced customs steps and twelve address-carrying legs from berth clamps to
bunk; **250,001 people over 19 roles**, each traceable (*Nadia Sinclair, human, EARTH, merchant,
23, home `qtr_civilian`* — Zocalo at 13:00, home at 03:00 and 22:00); corridors on a swept
shell, **props solid**, the drum on its real relief, **NPCs capsuled**; **50 of 129 places
bespoke**, 79 generic.

**What did not**, and it is now `docs/MASTER-PLAN.md` §P4a and §P4b rather than a session task
list — *which is where it lived for an hour, and a task list does not survive a context reset:*

| | | state |
|---|---|---|
| A4a-1 | the drum floor is empty at **4.5 million m²** — *"nothing standing anywhere"*, and five gazetteer rows report the IDENTICAL measurement | **open** |
| A4a-2 | **six subsystems at craft 1**, exactly one thing at craft 4 | **open** |
| A4a-3 | **the plant layer cannot fail** — C&C has a watch roster and controls nothing that can break | **agent running** |
| A4a-4 | **room occupants were dioramas** | **CLOSED** |
| A4b-1 | the interaction layer is a **wiggle, not a verb** | `read` closed; rest **agent running** |
| A4b-2 | **no inventory anywhere** | **agent running** |
| A4b-3 | **the economy is read-only in the game** | **agent running** |

### 22.1 THE ROOT CAUSE, and it is the owner's own session-4d ruling still standing

*The simulation is Python and the game is GDScript, and the bridge is ONE-WAY: Python bakes, the
engine reads.* Everything that passes a `--gate` and does not exist in the game is this. **85,940
lines of Python against ~17,000 of GDScript** — the counts got far better since 4d and **the
ratio has barely moved.**

### 22.2 INSTANCE TEN, and it is a number I had been repeating as fact

`main.gd::_configure_walk` sets `crowd_path` and **never** `crowd_glbs` or `crowd_ladder`, and
`walk.gd::_load_crowd_libs` line 591 is `if crowd_glb == "" and crowd_glbs == "": return false`.
**Every launch of the shipped build instanced ZERO corridor walkers.** The 963 walkers and
5,966 m quoted repeatedly in this file and to the owner were true of `walkable.py --deck`'s
command line and **had never reached the game**. Fixed (INV-412); now 8 walkers a cell.

**A static scan can tell you a caller exists; only running the thing tells you the caller runs.**

### 22.3 What landed

- **Room occupants are people.** 66 of 66 change state over a station-day; at 03:00 forty are
  away, twelve eat, seven work, four sleep; at 13:00 twenty-one idle, eighteen work, nine eat.
  Proven in the engine on the **streamed** path: `394 instanced occupant(s) with a timetable, 0
  baked into the deck mesh`, `occupants=11 changes=33 travel_m=245.8`. 249,728 triangles and 886
  primitives leave the deck mesh; people cost **31–33 draw calls against a budget of 32** — a
  rout that still straddles the line. Lost: within-species stature range, up to 352 mm.
- **The sleep clip**, and it needed **four IK contacts, not two** — posing arms by rotation put a
  sleeper's fingers 0.42 m in the air, because an elbow flexes *forward* and a 90° recline turns
  forward into up.
- **`read` is the first verb with a consequence.** Boards, plaques, monitors and atmosphere lamps
  now say things, all derived — the arrivals board reads from the same `signage.arrivals_lines`
  the mesh letters are cut from, and the monitors from `broadcast.day`, which had been
  **audible-only**.
- **SYS-15, the civic calendar.** 210 observances a station-week over 20 places. Its FROZEN
  control is the good one: force every rule day-independent and maximal and you get 1,302
  observances failing exactly one assertion — *"two consecutive days differ"*. **Renamed
  `calendar.py` → `civic_calendar.py` before commit: `station/` is on `sys.path`, so it was
  shadowing the stdlib.**
- **The Starfury docks.** 12 of 12 start phases over a rotation; peak 72.7% of the airframe;
  **the dock's contact velocity IS the launch's release velocity to 0.00198 m/s on 55.1483** —
  docking is launch run backwards and both read one bay model.
- **Spatial submission**: structure **4.34× → 3.00×**. The ladder in order of size is
  **residency, then the cell cut, then the occluder** — and the occluder alone moves whole-ring
  structure 604,610 → 603,318, i.e. nothing.

### 22.4 Two more headline numbers that were measuring a part and claiming the whole

1. `deck.py --sweep` calls **58,660 triangles "the whole walkable station"** — that is **ring
   decks only**. The drum's ground is 573,440, **91% of the real 632,100**. `budget.py` now
   prints all three.
2. `interact.py`'s **357/357 interactables** measures that they EXIST. Five of eight verbs
   "respond", and responding meant *the prop depresses for a few frames*. The module's header
   set out to end *"357 declarations, 0 verbs"*; it ended the declaration gap.

### 22.5 The owner asked about ragdolls, and the answer is one missing capability

`body.py` builds *"lofted rings around a fixed skeleton"* — but what ships is **baked pose
meshes**, and the runtime has **zero** `Skeleton3D`, `PhysicalBone3D` or skinning. Bodies draw
through **MultiMesh**, and a MultiMesh instance is a transform into a shared mesh: **it cannot
own a skeleton.** So a ragdoll needs **promotion** — hide the instance, spawn a real skinned mesh
with physical bones, demote after. It composes with "AAA detailed" because the body that
ragdolls is by definition the one being looked at, so it should be the **individual 7,212-tri**
body rather than the 484-tri shared one.
**One capability unlocks all three**: give `body.py` a skinned export (rest skeleton + weights)
and you get ragdolls, the individuality hybrid INV-403 says is blocked only by "the runtime
cannot skin on demand", and runtime blending instead of 12 baked pose slots — which would
*remove* the 32,288 tri/lod4 those slots cost. Catch: **14 species, 14 bone chains.** Task 19.

---

## Session 4n — THE STATION HAS FRICTION AND A NAV GRAPH, AND "BUILT BUT UNREACHABLE" IS FINALLY A GATE

### 21. READ THIS FIRST — the rule got gated, and the gate found three more instances in an hour

**The defect.** Finished, tested machinery with no caller on the shipped path. It had
happened five times before today — L3's room leg, `stream.gd` moving nobody, a circulation
graph nothing but its own selftest saw, `dialogue.gd`'s node that had never been built on any
path, and the Starfury. **Every one of them passed every gate in this repository at the time,
and they had to**: every gate here scores A PART AGAINST A STANDARD, and a part with no caller
still meets its standard.

CLAUDE.md's own rule says a fix applied to an instance and not to the rule is a fix that will
be needed again. Five instances is enough evidence. **`tools/wiring.py` asks the rule's
question**, in two halves, neither of which needs a build, a render or a GPU — it is file
scanning, seconds, and safe to run while agents are working:

| | question |
|---|---|
| `--data` | every `station/generated/…` path an engine script reads: **does the file exist, and is its producer run by CI?** |
| `--callers` | every tested module under `station/`: **does anything else in the repository import it** — by `import`, by attribute, or by `__import__("name")`? |

**It found the sixth on its first run.** `main.gd` read
`generated/scene/transit/transit_manifest.json` — a name appearing **exactly once in the whole
repository, on the line that read it**. Nothing wrote it. `transit_runtime.py --build` writes
`transit/lift.json` via `build_lift`, and `transit.gd` takes ONE manifest and branches on its
`kind`, so a combined index was never wanted at either end. `--mode=transit` could not have
worked, **and its error message named a command that would not have fixed it**: you would run
`--build`, watch it succeed, and get the identical error. Fixed to the name that is produced.

**The seventh** was `Director.route_between`, which returns an empty array unless `nav` is set,
and nothing in the shipped build set it. The agent that built the graph had `main.gd` on its
do-not-touch list, so it stopped at the boundary and handed over the patch — the process
working correctly, and still exactly how instances one through six were born. Four lines in
`main.gd` close it, wired as a soft dependency so a missing graph leaves `nav` null and the
station still boots.

**The eighth** is open and is the most valuable: `station/occluders.py` is **630 lines with a
selftest that nothing imports**, built specifically to close `budget.py`'s "structure at 2.05×
allowance" — the gate that has been red for four sessions and that blinded 34 CI steps in 4e.
It has `occluder_shell()` (built BY `collision.corridor_shell`, so there is no second
construction to drift), `gd_occluder()` emitting `ArrayOccluder3D` + `OccluderInstance3D` text,
and ray-cast verification that an occluder never hides something visible. Three ends to wire,
and the third is a correction: **`godot/project.godot` has no
`rendering/occlusion_culling/use_occlusion_culling`**, without which Godot ignores occluders
entirely, and **`occluders.py`'s own docstring claims `budget.py` "refuses to apply its
occlusion pass" — but `budget.py` has no occlusion pass at all**, only comments describing its
absence. The pass must be written, not merely enabled.

### 21.1 The tool corrected its author twice, which is the argument for having it

**The Starfury data exists and I said it did not.** I checked with
`ls station/generated/starfury/`, got "No such file or directory", and wrote it up as a
finding. The files are at **`station/generated/scene/starfury/`** — `main.gd` joins them onto a
base of `station/generated/scene`. An `ls` of a guessed directory cannot see that; a scan that
follows the actual read can. *The real Starfury defect is the weaker, truer one: no CI step
rebuilds that data, so it survives only because somebody once ran the builder by hand.*

**`--callers` reported 10 orphans and 8 were false**, from two rules I wrote myself:

1. `bespoke.py` dispatches with `__import__("shuttle").room(...)` — **a module name inside a
   string, invisible to an import regex**. This is the *identical* trap CLAUDE.md already
   records from session 4f, where `materials._scan_generator_groups` missed 45 mesh groups
   because `corridor_dressing` names them `f"dress_{kind}"`. I wrote a fresh instance of a
   defect this repository had already written down.
2. **"outside its own directory" measures nothing here.** I took the phrase from CLAUDE.md's
   "twelve tested modules with zero importers outside their own directories" — true and useful
   there, because it was about `station/npc/` and `station/physics/` as packages. ~60 modules
   share `station/`, so every intra-station import was being skipped.

10 → 2, and both survivors are real: `incident` (an agent's new module, correctly unwired) and
`occluders`.

**The gate is red today and that is correct.** MISSING fails; UNREPRODUCIBLE reports. A path
nothing satisfies is a mode that cannot run — unambiguous, so the gate goes red and stays red
until it is built. "On disk but no CI step rebuilds it" is the weaker claim, and failing on it
would take the gate red for six paths at once and bury the one that is actually broken.
Standing: **14 engine data paths, 1 missing, 6 unreproducible, 2 orphaned modules.** CI step
`swiring`, with its own id and `continue-on-error` — verified by parsing the YAML that all 53
id'd steps carry it, so no step can blind the ones behind it.

### 21.2 Faction friction — the arithmetic is the content

`friction.separation_m("narn", "centauri")` is **1.80 m**. `collision.corridor_shell` measures
blue/0/0's half-width at **1.0806 m**, so the widest gap two people can put between themselves
is **1.64 m**. **A Narn and a Centauri cannot pass in a ring corridor**, and the escalation is
not authored — it falls out of geometry the station already had.

Over one station-hour on that deck, 134 walkers: **30,250 encounters, 11,157 carrying a
grievance, 7,963 producing a world delta, 1,400.4 m of displacement** against a frictionless
twin run with the same people and the same seed. Lateral pass distance, the part a player would
see: 0.46 m with no grievance, 0.64 m with one, **1.14 m for a Narn and a Centauri**, 0.47 m
with the table off.

80 assertions across `faction.py --selftest` (27/27), `friction.py --selftest` (30/30) and
`encounter.py --gate` (23/23), all run here rather than taken from a report. **The before-state
is asserted, not described: the same nine content assertions against the pre-4n state report
9 of 9 FAIL.** Controls that fire: `PAIRS` emptied → 0 grievances, 0 deltas, 0.00 m;
`RESPONSES` emptied → the asymmetry collapses to widen/widen; S2E01 → 0 Nightwatch rows against
718 at the datum; 4× the timestep → 7,947 deltas against 7,963, 0.2% apart. CI step
`sfaction_friction`.

**Two honest negatives, recorded rather than buried.** The Markab ends-at control does *not*
fire (0 against 0 either side of Confessions and Lamentations). And **the Nightwatch row is 86%
of all friction on the station** — 14,286 of 16,683 grievance-carrying passes an hour — while
its own source says it fires only *"when an armband passes"*. The `witness=` condition exists
and defaults to the old era-only reading; only `encounter.py` passes the new one. Flipping the
default moves every human away from every alien on 128 baked decks, which is a re-bake and not
a patch. INV-312.

### 21.3 The navigation graph reaches the engine

```
NAVGRAPH PAIRS routed=741 of=741 mismatch=0 crossdeck=731 crossring=627
               crosssector=546 us_per_search=118.5 total_ms=87.8
```

All 741 pairs of walkable z-clusters route **at run time in GDScript**, each compared **node for
node** against `route_walk.path_between` — zero disagreements. A named resident walks it:
Malachi Vallo, centauri financier, `earthforce_office` → `medlab_green`, different z-clusters,
3 legs found *after* the scene was up, **641.6 m of 645.0 m covered on the floor, 0 frames off
it**, arriving 1.43 m from target against a 1.5 m bar — and `life.gd`'s Director returns the
identical 52-point polyline. Controls: no lift edges → **10 of 741** route, exactly the figure
`routes.station()`'s own docstring records for the pre-lift station; ring edges dropped → no
route at all; unsteered body → 449.11 m short. Artefact 179 nodes, 249 edges, 231,584 bytes.

**Four findings from that build, none of them fixed:**

- **The committed L3 agenda scene is stale by 57 m** and **the shipped `blue_0_0_col.glb` by
  65.5 m** — 4k's `room_interior_half_m` change flows into `corridor_z_m` and nothing re-baked.
  This is why the gate builds its own shells rather than trusting the committed ones.
- `routes.py` reports **`length_m` 0.0 on 249 of 249 built edges**, so metre-weighted search is
  currently inert.
- **GDScript `PackedInt32Array` is a VALUE**: `_adj[a].append(e)` silently discarded every
  edge. The graph loaded, ids resolved, and all 741 searches returned "no route" with no error.
- `route_walk`'s spine model runs *through rooms* between two clusters on one deck — on
  yellow/0/0 the transit angle is 0° and `fusion_core`'s bay is centred there, so the first walk
  stopped dead at z 310.7.

**Still open:** no body has ridden a **lift** on a runtime-found route. The graph routes across
decks (731 of 741 pairs); the traversal crosses z-clusters on one deck, because the only
committed cross-deck scene is the 57 m stale one.

### 21.4 PLAYTEST.md was stale in the expensive direction

Four of §0's eight "do not go looking for it" rows said ABSENT about things that work:
**dialogue** (157/157 exchanges offer a player line, 69 distinct player utterances), **the day
index** (`Clock.day()`, `day_offset`, `day_hour()`), **the Starfury** (false by grep —
`starfury.gd` has 34 references, plus a scene and 14 in `main.gd`), and **the economy** (the
bar's till debits; 267 → 420.50 cr over fourteen days, stock 52,720 → 51,518).

*A doc that says a thing is MISSING goes stale in the direction nobody re-checks. An absence
quietly becomes a standing instruction not to look* — so §0's four corrections were all found by
grepping the claims, which costs seconds, and the footer now says to re-grep before every
playtest.

### 21.6 P1-G2 and P1-G3 — standing you can lose, and a station that has incidents

Both built by agents, **both verified here rather than taken from their reports**, both with
their INV entries and CI steps applied by me because each was forbidden `INVENTIONS.md` and
`validate.yml` so their writes could not be lost against the other. Numbering verified
disjoint: G2 took **INV-340–349**, G3 took **INV-350–365**.

**G2 — `station/consequence.py`, `--gate` 34/34.** Six rungs summing to exactly 250,001
against `schedule.role_headcount()`, analytic rather than sampled. A rung is **not a field**:
`Player.tier` reads the card through `arrival.entry_class`, this project's one card reader, so
there is no second visa parser. Each rung gates places (128/126/116/66/35/33), counters
(13/13/13/10/9/4) and rent tier, and the gate asserts admittance is **monotone and literally
set-contained** — a tier that gates nothing fails.

Arrest → brig → fine → release closes with **every leg routed on the graph a resident commutes
on**: at the Zocalo a post is standing there so response is 0.0 min; from Downbelow it is
18.7 min from `council_chamber`. **It measures P-04 for the first time** — `LOCATIONS.md:448`
requires the brig be walkable from Security Central *and* the courtroom, authority 5, no source
placing it. Measured: 0.8 s and 74.9 s. The claim holds.

Deferral is **solved rather than chosen**: EA law is sovereign, so §4.2's jurisdiction conflict
*is* a non-EA defendant, and the arrested population is 78% human → 22%. The re-defer
probability then falls out of `P(hold ≥ 14 d) = 0.01`, and that one number reproduces **both**
of §3.1's brackets — median 15.4 h ("hours to a few days") and p99 13.6 days ("weeks").

**Two of its own controls did not fire on first run and both bought real fixes.** "Brig removed
from the register" passed silently, because the navgraph is cached and a deleted brig still had
a node. "Check rate ×100" changed nothing, because an avoidance term clamped to 1.0 and made
the check rate contribute **exactly nothing** to arrests — the constraint claimed to price
`CHECKS_PER_OFFICER_HOUR` was inert. Splitting the floor rung by role moved arrests 4.2 →
19.1/day.

**G3 — `station/incident.py`, `--gate` 24/24 in 1m36s, `--selftest` 14/14.** Twenty-two
classes and **not one authored rate**: every one comes from state that already existed
(`traffic.hall_rate`, `traffic.berths_in_use`, `security.hostility`, `audio.species_mix`,
`schedule.ROLE_WEIGHTS`, `arrival.checks` sampled through its own code path rather than by
copying its constants, `costume.ERA_EVENTS`). One headless station-day: **1,321 incidents in
16 of 22 classes, 2,117 world deltas, 135 in custody.** The six silent classes print *why*,
which is content rather than a gap.

**5.975 incidents/station-hour** in the probe at `customs_north`. The probe volume is
**topological, not metric** — the register place plus its declared adjacents, resolved once at
construction — precisely because SYS-14 says "never a floating radius an implementation can
shrink"; its 392 m span is an *output*. The witness radius is separate: 60.5 m from
`populace.corridor_sight_m`.

**The station-wide shape is printed honestly rather than cherry-picked: only 12 of 128 probe
volumes clear the ≥2/h floor.** The arrival half and Downbelow are alive (`docking_bays`
10.9/h, `downbelow` 7.1/h); the commercial ring is not (the Zocalo probe is 0.68/h) and the
residential rings are at zero. **That is the honest next target for content.**

Persistence is a **controlled A/B**, not an assertion: day 2 run twice from the same seed,
carried against fresh — 222 vs 122 in custody, grievance +0.5 vs +0.0, and **156 named facts
exist on the carried day that do not exist on the fresh one.**

**Four defects its own gates found in it**, and the first two are the transferable ones: the
MTBF sanity check **refuted its own derivation** at 0.04 days, because the denominator was 357
interactable *types* and `rooms.bays_in` gives 182,905 *instances*; and the step-size control
found **a rate, not a step** — 12.4% low at 4×, because the draw was one coin flip per step
(truncating at one event) *and* INC-DEBT was running at 44/h in one room. Also: **"M reachable"
was a tautology** — it simulated the probe alone, then asserted every incident was in the
probe.

**Still unwired, and logged before it becomes instance #9:** nothing imports `station/incident.py`
(`wiring.py --callers` flags it), and `consequence.certain_check()` exists with no runtime
caller on a door transition. Both are headless simulation today.

### 21.7 The lived-in half — 22 incident classes to 30, and the commercial ring wakes up

G3 printed its own honest shape and this acted on it. **12 of 128 → 18 of 129** probe volumes
clearing 2 incidents/station-hour; the **Zocalo 0.6817 → 4.632/h** with five more Red rows
crossing behind it; Red's mean ×5.8; one station-day **1,321 → 2,011 incidents in 24 of 30
classes**. `--gate` 33/33 in 2m20s, verified here after applying the eight spec rows the agent
was forbidden to write.

**The framing was the job and it is COUNTED rather than claimed.** `books_custody()` replays
every class absent/helps/reports and asks whether the brig gains a row in the **absent** branch
— what the station does when nobody is watching. **4 of the original 22 do; 0 of the 8 new ones
do.** Ordinary life is not a crime, and that assertion fails the moment somebody puts an arrest
in an absent branch.

**My brief was wrong and the correction is sharper than what I wrote.** I said the residential
rings were at ZERO; they ran 0.036–0.186/h. The true statement: **only two of the 22 could fire
in a home** — INC-FAULT (a fitting wearing out) and INC-DUST (a smuggling route crossing the
Alien Sector) — **and neither happens because somebody lives there.**

**The homes STILL do not clear the floor, and that is a finding with arithmetic behind it.**
`qtr_civilian` is 320 built households, so ≥2/station-hour is **one thing going wrong in every
flat every 6.7 days**. It runs at 0.709/h at its busiest hour — one per household per 19 days.
*The floor is a PORT's number*: everything that clears it moves 55 ships or holds 39,262 people.
And probe size is not the excuse — `core_shuttle` and `fusion_core` clear it with a single-place
probe.

**The best mechanism here was already in the repository and nothing read it.**
`schedule.RHYTHMS` says every Narn aboard is asleep at 03:00 (awake 0.000) while every Centauri
is awake (1.000); at 13:00 it is the Brakiri. `clock_mismatch` is the probability two residents
of a block are on opposite clocks — 0.298 at 22:00 in the Alien Sector, 0.274 at 03:00 in
`qtr_civilian` — so a residential corridor now has an **hour**: 03:00 0.707/h, 13:00 0.417/h,
22:00 0.951/h. *A dispute across a bulkhead between two species on opposite clocks is B5 content
that fell out of a table written for something else.*

**Two cross-checks the register produced that nobody had computed.** `rooms.bays_in` and
`populace.occupancy` are independent mechanisms, and divided they agree on **3.07 people per
unit** in every EA-standard block — a household — and diverge exactly where the content says:
**`downbelow` 9.23, `downbelow_arch` 14.29**, a 4.7× overcrowding ratio and the first
quantitative statement of that anywhere in this project. Separately: **2,800 medical staff
against 51 built beds is 55 staff per bed**, and the 13 residence rows hold **60,557 of 250,001
people (24.2%)**. Both shortfalls are *reported* rather than absorbed into a rate.

**The control that did not fire as intended is the best content in it.** INC-ARREARS is thin by
construction: day 1 stopped 110 earners and **0 of them live somewhere that charges rent**. The
households that lose an earner are exactly the ones `consequence.admits` turns away from a
medlab — SANCTUARY and NO_STATUS — and LAW-CRIME §7.1 prices their housing at 0 cr, *"and it is
why people are there"*. **THE SAME RUNG THAT DENIES YOU A BED MEANS YOU CANNOT BE EVICTED.** And
`economy.py` killed that class's first draft outright: a week's cheapest tenancy is 6.0 cr =
0.75 days of casual labour against a week's food at 31.5 cr = 3.94 days, so **rent is a fifth of
the food bill** and only a stopped earner can cause arrears.

**What was deliberately NOT made an incident**, by SYS-14's own test: meals, sleep and shift
changes belong to `npc/life.py` and are already carried; festivals, faith rotas, weddings,
funerals and drills belong to **SYS-15, the civic calendar, WHICH HAS NO CODE** — they are
calendar-shaped rather than rate-shaped, and an observance at a random hour is not an
observance. **A birth was refused outright and the refusal is a finding**: `ROLE_WEIGHTS` is a
*workforce*, so the model holds 6,253 children, no dependants, no schools and nobody's parents.

### 21.8 The Markab quarter, and the tiling recompute the spec had already asked for

**`markab_quarter` is a register place** (129 now). Three modules modelled it and the register
did not. Its acceptance check is the unusual one — **nothing is inside**; `crowd.py` asserts
headcount 0 at all 24 hours and `navigation.py::EXPECTED_ISLANDS` expects it **unreachable**, so
a router that "fixes" the island has broken something. An hour after it landed it became a free
control for the incident work: 10 built units, no residents, **0.0000/h of domestic incident**,
so a rate keyed to unit count rather than households present would fire in an empty tomb.

**`refugee_reception` is NOT a gap and adding it would have been the defect** — INV-367 records
the negative. It is one of **14 aliases among `WORKPLACE_FUNCTIONS`' 19 keys**; only five share
a name with a register row.

**THE UNIT ERROR IS THE TRANSFERABLE PART.** `footprint[0]` is **degrees of arc, not metres**.
The first draft copied `12.0` straight from `crowd.EXTENTS`, which at this deck's 292.700 m
radius would have built a **61 m frontage — five times the room the crowd model prices** — while
every document still said the two "agree". *Nothing in `directory.py` can catch that:
`collisions()` compares footprints against each other and never against the floor area another
module derives.* 12.0 m of arc at r = 292.700 is **2.3490°**. **A number copied between two
modules must be copied in the units the DESTINATION reads, and "they agree" is a claim about the
value after conversion, not before.**

**The tiling "drift" was one place.** `rooms.bays_in` summed to 51,465 against §TILING's frozen
49,265 — and the shape is what made it cheap: blue, red and grey **byte-identical**, green
differing by exactly the new place's 10 bays, and **all +2,200 in one row**, `core_shuttle`
4,000 → 6,200. **And the spec had asked for that recompute by name**: SPEC-CHANGE #2's own
recomputes list ended *"if the 4,000-bay target moves, THAT recompute lands here as its own
entry"*. The code landed in `a96a756`; nobody re-ran it; the address line was updated to 4,650 m
and the target was not. 4,650/3,000 = 1.55 and 6,200/4,000 = 1.55 — the whole delta is one stale
ratio. SPEC-CHANGE #2 closed, #3 and #4 written with their recomputes, PLC-129 added,
`completion.yaml` **regenerated** so `spec_check` reads 300 rather than a stale 291.

### 21.9 The occluder — and closing instance eight created instance NINE

`station/occluders.py` was 630 lines nothing imported, found by `wiring.py --callers`. It has
three importers now and the orphan list is down to one. **The wiring was not the job.**

**Its own self-test was RED at 6/7 with 209 containment breaches**, committed that way on
purpose. The fix retires three sessions of work and is pure arithmetic: `deep_profile` measured
the corridor by RAY CASTING — finer lattices, a door-bearing section, a 192-direction sphere
sweep — but a ray hit is `a + u(b−a) + v(c−a)` with `u,v ≥ 0, u+v ≤ 1`, **a convex combination
of the triangle's three vertices**, so it lies inside the kit's vertex box in every axis. No
cast can leave it; it can only fail to reach it, and where it fell short is exactly where
containment failed — **ceiling short by 340 mm, worst breach 169 mm**. `min()`/`max()` over the
vertices give the bound exactly, for free: **5m13s → under a second**, at a cost of 0.6 points
of sphere coverage. INV-370.

**Two of its controls could not fire and nobody had noticed.** Both aperture controls returned
the baseline's own 209 breaches, so at a 0 baseline they read 0. `ring_arc` builds **no room
behind a doorway**, and its `door_leaves` defaults `True` — doors SHUT, a configuration
`deck.build_deck` never builds. With `room_stub()` and open leaves they separate: sealed 34,
unwidened 2, widened 0. INV-374.

**The saving is small and is reported honestly.** Godot culls per **instance AABB**, not per
triangle (INV-371), and corridor groups span the whole 345° ring so their box contains the
camera. Structure 4.35× → **1.72× at triangle granularity, 10.06× → 10.06× at the granularity
that actually ships.** It does not clear 1.0× and nothing was tuned to pretend otherwise.
**`budget.py`'s old line — "what closes this is an occluder on the corridor's own walls" — is
half right and names the wrong half:** frustum submission at instance granularity is 998,014
against a per-triangle 547,731, and cutting the deck into the 18 cells `stream.gd` already bakes
takes it down **39% before any occluder**. That is task 14.

**AND THEN I CREATED INSTANCE NINE WHILE CLOSING INSTANCE EIGHT.** The load went into
`walk.gd::_load_level`, **which the shipped build never runs** — the shipped scene is STREAMED
and `_load_level` is the monolithic path. The scene booted, the occluder never loaded, nothing
said so. It is the trap that file's own header records for `stream.gd` and `dialogue.gd`.

**`budget.occlusion_chain` reported `applied=True` throughout**, because rung 3 looks for a
REFERENCE in the source and cannot see which branch runs. One level lower, `boot.json` had no
`occluder` key and `main.gd::_configure_walk` passed none, so the export var would have stayed
`""` even on the path that *did* call it. **Two silent breaks stacked underneath a green
chain.** Caught by launching the scene and grepping for the line the loader prints — the only
...
| # | work | blocked on |
|---|---|---|
| ~~1~~ | ~~place the sealed Markab quarter and `refugee_reception`~~ | **DONE (§21.8)** — and only one of the two was a gap |
| 2 | close **INV-312** (Nightwatch witness) | a 128-deck re-bake; needs free cores |
| 10 | **the certain-check trigger on a door transition** | `consequence.certain_check(place)` is a pure per-place predicate and **nothing in the runtime calls it**, so visa revocation is reachable in Python and not in the game. The wiring is small — `places.gd` already resolves which place the player is in from mesh AABBs, and both `hud.gd` and `ambience.gd` consume it — but it needs a Godot run to verify. **Logged BEFORE it becomes instance #9** |
| 13 | **SYS-15, the civic calendar** | enumerated in `docs/spec/SYSTEMS.md`, harness reads `tool-to-build ⇒ RED`, no code. The largest remaining piece of ordinary life, and deliberately *not* forced into the incident table |
| 6 | **bake the Starfury data in CI** and fly a **dock phase** | P4, and the owner redirected this session to the living station. Launch is built; docking is *analysed* (`--docking-envelope`: ceiling at **227.8 m** of standoff, beyond which ω²R exceeds the airframe's maximum) and never flown |
| 7 | **re-bake the stale committed scenes** (57 m / 65.5 m) | minutes of 100% CPU |
| 8 | **wire `occluders.py`** + `project.godot` + write `budget.py`'s occlusion pass | agent-sized; closes a four-session red |
...
**CORRECTION (2026-08-04, at completion-bible adoption): the totals above are superseded.**
Commit c4f989b (V1, `rooms._fit_bay`) fits bays to their footprints instead of deriving one
span per room, and the live recompute is **49,265 bays wanted, not 73,635** (blue 7,692 /
red 1,644 / green 7,052 / grey 16,487 / yellow 16,390), with `docking_bays` at **1,092, not
2,052**. The per-place normative targets live in docs/spec/PLACES.md (TILING is frozen there;
recompute divergence is a SPEC-CHANGE). MASTER-PLAN A5's "73,507 unnamed bays" read THIS
section's old total as bays outside the named places — they ARE the named places' footprint
bays; A5 carries its own dated supersession note. The streaming specification above (near
field at full depth, dress only within sight, state the cap loudly) is unchanged and is now
normative via PLACES §TILING rule 3.
...
## 26. Session 4s — three recycles, a lost agent, and a workflow that failed at the wrong layer

**This session produced one rule, one standing defence, and one workflow that is still running as
it is written.** No station content changed. That is an honest summary and the reason it is worth
a section is that all three are about *keeping work*, which this project had never had to think
about before.

### 26.1 The container recycled three times, and the recovery is two commands

Each recycle rolled `/home/user/Opus-5` back to a commit several hours old, wiped `/tmp`
(including the workflow script the Workflow tool had persisted there), and deleted every
`git worktree`. The task list reverted too.

**The main agent lost nothing, all three times**, because of one habit:

```
for i in 1 2 3 4; do git fetch origin <branch> && break || sleep $((2**i)); done
git reset --hard origin/<branch>
```

That works **only** if the work was pushed. It was, within a minute of every increment landing.

**An agent lost everything.** The collision-shell agent had the brief at
`scratchpad/BRIEF-shellfit.md`, was working the `deck.py --shell-fit` RED (20 rooms with geometry
outside their own collision shell), and had committed inside its worktree. Nothing reached the
remote. `git log origin/<branch> | grep -i shell` returns nothing. The run is gone.

**So the rule, and it is one word longer than 4r's rescue-snapshot rule:** a build agent's brief
must say **PUSH**, not commit, and say it should push *early and often* rather than once at the
end. A worktree is the right answer to artefact contention and the wrong place to keep anything.

`scratchpad/push_mirror.sh` is the standing defence: a `nohup` loop that pushes whenever `HEAD`
moves and **never creates a commit**, so the worst it can do is publish a checkpoint somebody else
made — which is exactly what should survive a recycle. It costs nothing. **Restart it at the top
of any session** (`nohup nice -n 19 sh scratchpad/push_mirror.sh &`); a recycle kills it.

**And put anything a workflow needs in the repo, not `/tmp`.** The Workflow tool persists its
script under the session directory, which is on `/tmp`, which the recycle wiped. The relaunch
works because `scratchpad/wf-craft-4s.js` is committed and pushed; after the third recycle the
file came back with the checkout.

### 26.2 The workflow failure named the wrong layer, and the journal said so

The first launch of the blind-craft workflow returned:

```
pipeline[0..4] failed: agent({schema}): StructuredOutput retry cap (5) exceeded
5 agents, 0 done, 5 error, 309,811 subagent tokens, 295 s
```

That reads as a schema defect. It was not. The journal — which the tool's own diagnostics say to
read *before* diagnosing — shows every tool call in every agent rejected **before execution**:
...
Reproduced by one agent across five tools with a table: `Read`/`file_path`, `Bash`/`command`
(including a bare `pwd`), `Glob`/`pattern`, `Grep`/`pattern` — **and `StructuredOutput` itself**.
So the agents could not do the work and could not report that they could not do the work; the
retry cap was the *symptom of the reporting channel being broken*, not of the schema.

Two things worth carrying:

* **The agents behaved correctly under total failure.** One wrote the diagnostic table, and when
  forced to return an object it set `ok: false` and wrote *"Quoting zeros would be manufactured
  evidence."* That is the right instinct and it is the same one this file demands of gates.
* **`required` was too long.** Nine required fields meant a blocked agent's every retry failed on
  nine counts instead of letting it say *I am blocked*. The relaunched schema requires only the
  five fields the control flow branches on, and the brief now tells an agent to report a harness
  fault and stop rather than burn context on it.

It was transient. A one-minute probe agent (`pwd`, `head`, `Read`) came back three-for-three clean.

### 26.3 What is running right now, and what to do if it is gone

**Workflow `wf_a057ccef-d30`**, script `scratchpad/wf-craft-4s.js` (committed — read it, it is the
brief). Five subsystems, chosen worst-craft-first off `docs/aaa-scorecard.json`, file lists
disjoint and checked for artefact collisions:

| subsystem | owns | why |
|---|---|---|
| `generated_rooms` | `rooms.py`, `dressing.py` | **58% of the station**; fidelity 2 |
| `npc_bodies` | `npc/body.py`, `costume.py` | lowest row on the board on **all four** dimensions |
| `interior_lighting_4b` | `export_scene.py` | 13 of 23 on the distribution test; ambient owns p5 |
| `observation` | `observation.py` | a window onto space that renders **black** |
| `drum_interior_engine` | `drum_ground.py`, `drum_dressing.py` | the payoff view, "still bare underfoot" |

Five and not six because `observation_domes` and `observation_rotunda` are two scorecard rows and
**one source file**.

**The method is the point.** Each build agent renders BEFORE at the rubric's three distances,
reworks, renders AFTER at *identical* cameras, then copies both into
`scratchpad/blind/<key>/r<N>/` as `left-*.png` / `right-*.png`, choosing the mapping itself and
writing it **nowhere on disk** — it returns only in the structured value. A hostile critic gets two
unlabelled frames, is forbidden `git log`/`git diff`/build notes, must declare `did_not_peek`, and
is told to default to *no*. If it does not pick the new build, or picks it and still says not-AAA,
the builder gets the critic's own words back and goes again. Max 2 rounds this batch.

**This exists because every craft gate in the project scores a frame against a descriptor, and a
descriptor is a thing an author can talk themselves into.** A critic that cannot tell which frame
is new cannot be agreeable.

**If the workflow is not running when you read this**, it was killed by a fourth recycle. Relaunch
it: `Workflow({scriptPath: "/home/user/Opus-5/scratchpad/wf-craft-4s.js"})`. Check
`scratchpad/blind/` and `git log` first — an agent that pushed before dying left real work.

### 26.4 The next batch, in order

1. **Re-run the collision-shell fix.** `python3 station/deck.py --shell-fit` is landed and **RED at
   20 rooms**; `ambassadorial_suites` has a ±5.25 m shell against a mesh running −92.28..+8.53 m.
   The brief is at `scratchpad/BRIEF-shellfit.md`. It collides with `generated_rooms` on
   `rooms.py`, which is why it is not in batch 1. **Tell it to push.**
2. Whatever batch 1 leaves short — the workflow returns `still_short` and each row carries the
   critic's `worst_defect` and `what_would_fix_it`.
