...
**Last updated:** 2026-08-07 · **Session 4t — THE PACKAGED GAME WAS A TEST FIXTURE WITH NOBODY
IN IT, AND EVERY GATE IN THIS REPOSITORY SCORES THE DEV TREE. READ §1 ABOVE ALL.**
The artefact a player downloads was launched for the first time this session and printed
`MONOLITHIC -- no cell set`, `cast of 83`, and `could not load any crowd library`. Three stacked
defects: `boot.json` named the walk-test deck instead of the streamed one (**408 actors and 444
crowd placements** sat in a directory `boot.decks()` could not see, because it enumerates
`*_col.obj` and only the walk-test path emits that name); `crowd_lod*.glb` had **never been
generated by any build path**, which `walk.gd`'s own docstring already called instance ten of the
signature defect; and `main.gd` set `crowd_path` without the LOD ladder, so the fallback drew every
walker with the 400 m body at arm's length. Fixed and gated — `boot: STREAMED -- 103 cells`,
`cast of 408`, 1,632 dialogue exchanges — and `package.sh` now **destroys the build** if the crowd
library fails to load or the cast is zero. **Nine rounds of craft review scored frames from
`render_shot.gd`, which builds bodies in the dev tree and never reads that library at all.**
· **AND THE PREVIOUS SESSION'S LEAD, WHICH STILL BINDS:** **Session 4s — A CONTAINER THAT RECYCLES
IS A CONTAINER THAT DELETES YOUR AGENTS' WORK. READ §26.**
...
## Session 4t — THE SHIPPED BUILD WAS A TEST FIXTURE WITH NOBODY IN IT, AND NINE ROUNDS OF CRAFT REVIEW COULD NOT SEE IT

**Read §1 before planning anything. The rest of this file is history; this section is the
handoff.**

### 1. WHAT WAS ACTUALLY WRONG — one symptom, three stacked causes

The session ran nine build→judge items over five rounds and scored them 2s and 3s. Then the
packaged artefact was launched — **for the first time in the project's history** — and printed:

```
main: MONOLITHIC -- no cell set for blue_0_0_z7440
walk: cast of 83
ERROR: walk: could not load any crowd library
walk: 83 room occupant(s) have a timetable and NO shared body library -- they cannot be drawn
interact: no ledger at ../station/generated/economy.json -- nothing can be bought
```

A 325 MB download of a station simulation **with no people in it**. Three separate defects:

| # | defect | why nothing caught it |
|---|---|---|
| 1 | **`boot.json` named the wrong build.** `scene/deck/` is what `walkable.py` writes as a side effect of a WALK TEST — one z-cluster of six, 83 room occupants, no corridor crowd, no cells. `scene/station/` is the real deck: 6 clusters, 23 rooms, 4.38 M tri, **408 actors, 444 crowd placements, 206 baked cells**. | `boot.decks()` enumerates `*_col.obj`; only the walk-test path emits that name. The real build was **invisible** to the file that writes the manifest. Nothing chose the small one. |
| 2 | **`crowd_lod*.glb` had never been generated by any build path.** Written in exactly one place — inside `walkable.py::_bake`, under `if crowd:`, during a walk test. `deck.py`'s build path never called it. Dev checkout had **zero**. | This is **instance ten** of the signature defect, and `walk.gd`'s own docstring already called it that. Nobody read the docstring against the artefact. |
| 3 | **`main.gd` set `crowd_path` and not the ladder.** `walk.gd` has a directory-scanning fallback that loads what it finds under a synthetic `1e9:8` — so all 444 walkers get the **400 m body at arm's length**. | The fallback *works*, so nothing errored. A silent downgrade, which this project has now shipped four times (OpenGL, LOD ladder, monolith, exposure). |

**Measured, dev build, before → after:**

```
MONOLITHIC, no cell set   ->  STREAMED, 103 cells, 17 z-bands of 73.8 m
cast of 83                ->  cast of 408
could not load any crowd  ->  4 walkers per corridor cell, no error
(no ladder)               ->  18:2,45:4,400:8
(no dialogue sidecar)     ->  1,632 exchanges, 363 of 408 people can speak
```

### 2. WHY NINE ROUNDS OF CRAFT REVIEW SAW POPULATED ROOMS

**Every judge scored a frame from `render_shot.gd`, which builds bodies directly in the DEV TREE
and never consults the crowd library at all.** The reviewed frames were full of people. The
shipped artefact was empty. This is this project's own rule — *a thing is built more than once
here, and a gate on one build path says nothing about the other* — landing on **the one path that
ships**, after being written down twice for `quarters` and `plant`.

**The general form, and it is the expensive one:** every gate in this repository scores the
DEV TREE. `walkable.py`, `deck.py --sweep`, `density.py`, `materials.py`, `budget.py`,
`interact.py --audit`, every craft judgement. **Not one of them had ever launched the artefact a
player downloads.** Nine subsystems can be reviewed to a 3 and the download can still be a
different program.

### 3. THE COMMENT THAT KEPT THE HOLE OPEN — a differential is blind to symmetric breakage

`package.sh` had a differential gate (packaged run vs source run, diffed) and an explicit written
argument for why it did **not** assert on errors:

> *"there IS a standing error on both sides of the A/B — `walk: could not load any crowd
> library` — because no `crowd_lod*.glb` has ever been generated. A gate that failed on any error
> would fail for a content gap packaging did not cause and cannot fix."*

**Every clause is true.** The conclusion turned a known-missing feature into a permanent
exemption, and the differential could not see it **because the source tree was broken the same
way**. A differential is blind to anything broken identically on both sides.

*A line excused as "the source does it too" needs the second question asked out loud: **is the
source tree right?*** Here it was not, for nine sessions.

### 4. WHAT LANDED

| what | where | gate |
|---|---|---|
| the shared crowd library, all 3 ladder rungs, 6 s, no deck needed | `tools/bake_crowd.py` | `--selftest` asserts **per rung**, not a count; negative control on an empty dir exits 1 |
| GLB→OBJ, bridging the two builds' naming conventions | `tools/glb_to_obj.py` | 20,196 tri out — **exactly** the deck manifest's own `collision_tris` |
| `--deck-dir`, the crowd ladder in the manifest, `deck_dir` recorded | `station/boot.py` | `boot: STREAMED -- 103 cells` |
| `crowd_ladder` / `crowd_glbs` reach the engine | `godot/scripts/main.gd` | the `no crowd library was named` warning is gone |
| dialogue (1,632 exchanges) and arrival sidecars for the real deck | `scene/station/` | arrival spawn cross-checks the collision shell to **0.000 m** |
| **is anybody in it** | `tools/package.sh` | destroys the build on `could not load any crowd library` or a cast of 0 |
...
### 6. THE PROCESS COST, STATED PLAINLY

The session spent most of its budget on a **build→judge loop with no exit condition** — nine
items, five rounds, judges scoring against a bar of 4 and returning 2s and 3s. `AAA-STANDARD.md`
warns about exactly this in writing (*"a sufficiently harsh critic always finds something, so
'keep going until it's AAA' without a defined bar never terminates and one item eats unbounded
budget"*) and the rule was read and then not applied. **Round 5 additionally burned 132k tokens
returning nothing**, to the session-4s harness fault (a permission layer stripping parameters off
every tool call), relaunched into without checking the harness was healthy first.

Against that: **122 commits, 24,157 lines across 35 files**, all pushed. When the worker restarted
and killed the in-flight workflow, **31 worktrees held exactly one unpushed commit set** — the
push discipline from 4s held.

**The rescue had a trap worth carrying forward.** `wt-obs` had branched before another agent
landed, so `git diff origin..HEAD` showed it deleting 495 lines of `tools/export_scene.py` and six
blind frames. **A plain merge would have reverted that agent entirely.** Cherry-picking each
commit's own diff kept both. The two agents had also both allocated **INV-1220 and INV-1221 for
different inventions** — renumbered to 1222–1226, cross-references included, `inv_check` clean at
434.

*When rescuing work from a stale worktree, read each commit's own diff. `origin..HEAD` on a stale
base describes everything that landed meanwhile as a deletion.*
