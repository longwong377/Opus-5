...
| # | work | blocked on |
|---|---|---|
| 1 | place the **sealed Markab quarter** and `refugee_reception` in the register | both are modelled crowd/schedule entities (`schedule.py`, `crowd.py`, `resident.py`, and `dialogue.py` already speaks Narn lines at the reception) with **no walkable place in `directory.PLACES`**. Adding two rows takes it 128 → 130, which the running agents denominate against |
