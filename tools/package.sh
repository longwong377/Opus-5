...
#
# WHY THESE TWO AND NOT A GENERAL "no ERROR lines". Because there IS a standing
# error on both sides of the A/B -- `walk: could not load any crowd library`,
# which the SOURCE TREE prints too, because no `crowd_lod*.glb` has ever been
# generated. A gate that failed on any error would fail for a content gap
# packaging did not cause and cannot fix, and would then be switched off. These
# fail only for a difference between the source tree and the download.
# ---------------------------------------------------------------------------
diminish() {
  say ""
