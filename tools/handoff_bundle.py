...
def materials_table():
    ...
    for name in dir(M):
        ...
        if isinstance(sample, dict) and {"albedo", "roughness"} & set(sample):
            out[name] = v
    return out
...
def main():
    ...
    with open(os.path.join(a.out, "materials.json"), "w", encoding="utf-8") as f:
        json.dump(materials_table(), f, indent=1, sort_keys=True, default=str)
    ...
    print(f"\n{len(manifest['places'])} files, {tot/1048576:.1f} MB, "
          f"{manifest['total_triangles']:,} triangles -> {a.out}")
    if manifest["failed"]:
        print(f"{len(manifest['failed'])} could not be built:")
        for r in manifest["failed"]:
            print(f"   {r['key']:22} {r['why']}")
    return 0
